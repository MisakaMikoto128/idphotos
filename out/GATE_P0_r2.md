...
> 该产物**没有** `inputProvenance` 这个键，`tools/` 与 `lib/` 里也不存在该标识符）。
...
> （另注：该产物**没有** `inputProvenance` 这个键 —— 顶层键是 `provenance`；
> `tools/` 与 `lib/` 全库检索 `inputProvenance` 命中 0。**引字段名也要先在盘上找到它。**）
...
**8/8 严格单调递减。**（8 个点随机排序恰为单调的概率 = 1/8! ≈ 1/40320。）
...
> **但本设计里"输入倾角"与"制备旋转量"是同一个 `deltaDeg` 的两个折叠，无法分离**，
> 故**不指认成因**。
