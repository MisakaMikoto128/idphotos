...
> ① 的**后半句仍然有效且独立**：`c05` 家族**同为旋转件**、跨 Δ 无单调趋势
> （0.360 / 0.429 / 0.323 / 0.272），这一条**不依赖**"有没有旋转"的前提，故保留。
...
**"该项可能为制备/旋转诱导的伪影，成因待查"**；
**别把它写成生产缺陷，也别反过来拿它给 `c06_d-3` 打掩护**
（`c06_d-3` 眼区干净、不在洞名单，是**无争议的引擎失败**）。
...
`|制备时施加的旋转|` = |`deltaDeg`|（c08 基准 −8.01°）；洞取 `P0_alpha_holes.json`
的 `controlComparison.rotatedFixtures.perFixture`：
...
**但 §14.5 的措辞规则要加强**：既然旋转重新成为首要候选，
凡依据含"眼区有洞 / 量具量不出"的条目，注明 **"可能为制备/旋转诱导的伪影，成因待查"**，
**不许写成生产缺陷，也不许写成"已排除"**。
