...
`out/P0_anchors/*.png` 是**引擎真正吃进去的输入**。同源同 0° 对照（qa-batch `d66de4d`，主会话复算）：

| 输入 | 尺寸 | 眼带零占比 |
|---|---|---|
| `Pictures/Camera Roll/WIN_20230522_00_19_11_Pro.jpg`（**原图**） | 1280×720 | **0.0** |
| `out/P0_anchors/c08_upright.png`（**同源、同 0°、经夹具制备**） | 1280×720 | **0.6193** |

⇒ **旋转被排除**（0° 也有洞），**源图属性也被排除**（原图 0.0）；剩下的是**制备路径**。
故：**凡某条违规的依据里含"眼区有洞 / 量具量不出"，就注明"该项可能为制备伪影"**；
**别把夹具伪影写成生产缺陷，也别反过来拿它给 `c06_d-3` 打掩护**
