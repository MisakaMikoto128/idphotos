...
> **判决数字的适用域（勘误 24，引用任何一条结论前必读）**：
> **本报告一切计数都只对判据声明节写明的那个 revision 有效。**
> 具体到 P0.3b：**`= 2` 不含 `(1.5°, 3.0°]` 带内样本在 `27670b3`(2026-09-17 00:37, 死区 3°→1°) 之前的历史违规**
> —— **凡 `|tilt| ≤ 3°` 的照片在旧代码上"一点都不摆正"（恒等变换）**，
> 该 commit 已修掉，故那一段在当前 revision 上**不可测**。**不得把"P0.3b = 2"读成"摆正问题只剩这两条"。**
> **该带的规模要带限定词**：**P0.3b 分母内 8 条**（`corpus=='rotated'`）；
> **同带另有 2 条 `anchor`**（`c06` −1.730、`c12` −2.090）**不在该分母里**；**合计 10 条**曾落在旧死区内。
> **裸的"8"会被读成"盲区的全部"** —— 该带当前 **10/10 均已正确摆正**（复算，非采信提交信息）。
...
**判据明确不覆盖的属性（**与「扫过、0 命中」是两回事**，故单列；勘误 25）：**
- **「成片看起来被切了一刀」没有任何判据覆盖 —— 本门禁对此没有读数。**
  G4 的 2B.9 判据是"**图外区域出现的非底色像素总数 = 0（阈值 0）**"，**越界本身被明确允许**
  （越出部分按 alpha=0 填底色，填的就是底色，判据当然为 0）。
  设计容忍上限 `maxOutOfBounds = 0.45`（`lib/core/imaging/crop_geometry.dart:416`；
  G4 自检也照此断言，`dev_repro_g4.dart:162`）。
  用户自己的照片 `p1` 越界 **15.46%**，远在容忍内故**不触发缩小**，成片上留下斜切口。
  **判据问的是"图外有没有非底色像素"，用户看的是"身体被斜着切断了" —— 判据的范围窄于用户可见属性。**
  **本门禁不断言该不该收紧**（那是产品决定，可能要走 `docs/ACCEPTANCE.md`，而它只有主会话能改、
  且只在人工明确要求时改）。**此处只报事实：这道口子现在是敞开的，且在判据里看不见。**
  **沉默比缺失更容易被当成"没问题"，故显式写明。**

**机器可读旁注**：本轮的适用域与覆盖缺口另有一份**只增不改**的 `out/GATE_P0_r2.caveats.json`
（不改 `out/gate_P0_r2.json` 那份原始记录）—— **只读 JSON 的下游应同时读它**；
**限定词写在没人读的载体上等于没写。**
...
> **2026-09-17 更正（勘误 26）：上面这组数取自一个已被取代的 revision，且中位口径未署名。**
> **不是"编出来的"，是"过期 + 没署口径"——两者都构成作废，但更正的话要写准。**
>
> 该行四个数逐个查过（`out/P0_compose_items.jsonl` 的**两个**提交版本，逐条现算）：
>
> | 数 | 本行写 | `f6e7465`（旧版） | `affbac8` / 当前盘面（新版） | 判定 |
> |---|---|---|---|---|
> | `>0` | 73 | **73** | 67 | 旧版成立，**新版不成立** |
> | `>0.10` | 40 | **40** | 39 | 同上 |
> | `>0.20` | 26 | **26** | 23 | 同上 |
> | 中位 | 0.0423 | **0.042251**（`sorted[n//2]`，偶数 n 取上中位）／0.034116（`statistics.median`） | 0.037018 ／ 0.031794 | **旧版 + 上中位口径**才成立 |
> | 最大 | 43.5% | `c05_upright` 0.4354 | `c05_upright` 0.4354 | **两版均成立** |
> | `p1` | 15.4% | 15.45% | 15.46% | 两版均成立 |
> | `p2` | 2.1% | 2.13% | 2.13% | 两版均成立 |
>
> **两处错，性质不同：**
> 1. **没有 revision 戳**。`73/40/26` 是 `f6e7465` 时点的真值，`out/P0_compose_items.jsonl`
>    其后被重新生成，计数变 67/39/23。**本行没写它属于哪一版**，于是同一个数字在报告里
>    既"曾为真"又"现在为假"，而**读起来完全一样**。这正是本项目那个特征形态。
> 2. **中位口径未署名**，而它在这份数据上**真的会变**：n=100 为偶数，
>    `statistics.median` 取两中位之均、`sorted[n//2]` 取上中位，**差 0.0081**（0.0341 vs 0.0423）。
>    **同一份数据，两个都叫"中位"，差得比很多判据余量还大。**
>
> **处置：本行四个数按"数不出处即作废"处理**（同 `:209-212` 的先例），且**不得**被后续轮次引用。
> **但更正口径要写准**：它们**不是**"在盘上任何产物任何口径下都复现不出来"——
> 它们是**在一个已归档的旧 revision 上、按一个未署名的中位口径**可以逐条复现的。
> 「复现不出来」与「取自被取代的版本」是两种不同的失效，**更正的措辞若混为一谈，
> 就更正成了一句新的错话。**
> **量具教训**：凡报中位数，**必须连口径一起报**（`statistics.median` 还是 `sorted[n//2]`），
> 并注明**所据产物的 revision**。见 `docs/PITFALLS.md`。
...
| 项 | 期望 | 实测 | 责任 agent |
|---|---|---|---|
| P0.3b 旋转夹具条件覆盖率：真值 |tilt| > 1.5° 的夹具上 unavailable 必须 = 0 | 需要摆正的 67 条夹具上 unavailable = 0 | 违规 2 条（需要摆正 /truth/ > 1.5° 却返回 unavailable）：c06_d-3(truth -4.730°)、c08_d-10(truth -18.010°)——这 2 条成片未施加任何旋转，仍歪着对应角度。**⚠ 口径限定（勘误 24）：此计数只对当前 revision 有效** —— 它**不含** `(1.5°, 3.0°]` 带内样本在 `27670b3`(00:37) 之前的历史违规（旧死区 3.0° 吞掉整段，现已在当前 revision 上不可测；经复算**当前全部已正确摆正**）。该带**在 P0.3b 分母内有 8 条**（`corpus=='rotated'`），**同带另有 2 条 `anchor`**（`c06`/`c12`）不在该分母内，**合计 10 条**曾落在旧死区内。**不得把"P0.3b = 2"读成"摆正问题只剩这两条"** | ml-porting |
...
| 项 | 期望 | 实测 | 责任 agent |
|---|---|---|---|
| P0.3b | 真值 \|tilt\| > 1.5° 的 67 条夹具上 `unavailable` = 0 | **违规 2 条**：`c06_d-3`(truth −4.730°)、`c08_d-10`(truth −18.010°)，两条成片**未施加任何旋转**，仍歪着真值角度。**⚠ 此计数只对当前 revision 有效**（勘误 24）：不含 `(1.5°, 3.0°]` 带内在 `27670b3`(00:37) 之前的历史违规 —— 该带 **P0.3b 分母内 8 条**（`corpus=='rotated'`）、**同带另有 2 条 `anchor`**，**合计 10 条**曾落在旧 3° 死区内，当前均已正确摆正 | ml-porting |
...
# 勘误 24：`P0.3b = 2` **只对当前 revision 有效** —— 死区 3°→1° 之前的整段历史违规不可测

## 24.1 那条"死区设计"其实是一次 P0 修复，而且有历史

§22.2 我把 `kRollDeadZoneDeg = 1.0` 写成"设计如此、且有名的"。**对，但只说了一半 —— 它有过另一个值。**
主会话指出后我自查（`git log -L125,125:lib/core/imaging/crop_geometry.dart`）：

```
0634c42  2026-09-08 07:26  +const double kRollDeadZoneDeg = 3.0;   ← 引入
27670b3  2026-09-17 00:37  -const double kRollDeadZoneDeg = 3.0;
                           +const double kRollDeadZoneDeg = 1.0;   ← imaging
                           主题："imaging: P0 歪斜修复——摆正死区 3°->1°（小角度盲区根因）"
```

**归属是 `lib/core/imaging/` —— imaging 的地盘**，不是 ml-porting 的。**它是一次修复，不是原始设计。**

## 24.2 口径后果（**我改的是措辞，不是判决**）

P0.3b 的分母是**真值 `|tilt| > 1.5°`**。**旧死区 3.0° 会把 `(1.5°, 3.0°]` 整段吞掉** ——
那段在 `27670b3` 之前是"**真·施加 0°、真值确实在分母里**"，**本该是违规**。

**但门禁测不到它**：qa 的成片 07:34 才跑，**已含 00:37 的修复**。

**该段有多少条？我数了（`out/P0_truth.json` → `rotated`）：**

- 旋转夹具总数 **78**（`corpus == 'rotated'`，即门禁 `gate_P0.dart:570` 的口径），`|真值| > 1.5` 即 P0.3b 分母 **67**（与判据表一致）；
- **落在 `(1.5°, 3.0]` 的：8 条** —— `c01_d+3`(1.700) `c02_d+3`(2.750) `c07_d+3`(2.650)
  `c08_d+10`(1.990) `c10_d+5`(1.560) `c11_d+3`(2.890) `c12_d+5`(2.910) `p2_d+3`(2.800)。

> **⚠ 限定词（主会话指出，我已复算）：这个"8"是"落在 P0.3b 分母内"的数，不是"旧死区吞掉的样本总数"。**
> **不分 corpus 地枚举 `(1.5°, 3.0]`，共 10 条** —— 除上述 8 条 `rotated` 外，**另有 2 条 `anchor`**：
>
> | id | `corpus` | 真值 | 当前施加 | 残余 | `rollSource` |
> |---|---|---|---|---|---|
> | `c06` | **anchor** | −1.730 | −1.7106 | 0.0194 | pupil |
> | `c12` | **anchor** | −2.090 | −2.1435 | 0.0535 | pupil |
>
> 它们**与那 8 条同带、同样在旧 3° 死区里被完全跳过**，只因 `corpus` 标成 `anchor`
> 而**不在 P0.3b 的分母里**（门禁分母是 `corpus == 'rotated'`）。
>
> **⇒ 正确写法**：8 条**在 P0.3b 分母内**；同一带内另有 2 条 `anchor`，**不在该分母里**；
> **合计 10 条曾落在 3° 死区内。**
> **一个裸的"8"会被读成"盲区的全部"** —— 这正是本节 24.4 那条规则的现场自指：
> **数会脱离上下文传播，限定词不会。**

## 24.3 顺手把 imaging 那次修复**验了一遍**（这是新的正面证据）

那 8 条在**当前 revision** 上的实际表现（`out/P0_compose_items.jsonl`，`cn_big_1inch`）：

| id | 真值 | 实际施加 | 残余 | `rollSource` |
|---|---|---|---|---|
| `c01_d+3` | 1.700 | 1.9041 | 0.2041 | pupil |
| `c02_d+3` | 2.750 | 2.7830 | 0.0330 | pupil |
| `c07_d+3` | 2.650 | 2.8168 | 0.1668 | pupil |
| `c08_d+10` | 1.990 | 2.1707 | 0.1807 | pupil |
| `c10_d+5` | 1.560 | 1.4021 | 0.1579 | pupil |
| `c11_d+3` | 2.890 | 3.4269 | 0.5369 | pupil |
| `c12_d+5` | 2.910 | 2.9500 | 0.0400 | pupil |
| `p2_d+3` | 2.800 | 2.9827 | 0.1827 | pupil |

**⇒ 8/8 残余 ≤ 1.5°（最大 0.537）、8/8 `rollSource = pupil`。**
**旧盲区里的 8 条，当前已全部被正确摆正** —— `27670b3` 的效果**是可测的、我测了**。

**同一带内的 2 条 `anchor`（`c06` −1.730 → 残余 0.0194；`c12` −2.090 → 残余 0.0535，均 `pupil`）同样已摆正。**
**⇒ 曾落在旧 3° 死区内的 10 条，当前 10/10 均已正确摆正。**

**带内唯一的异常是 `c11_d+3`：施加 3.4269 > 真值 2.890，残余 0.5369 —— 它是"转多了"不是"转少了"。**
**阈值内（< 1.5），不改判**；记账时注明方向即可。

## 24.4 加在判决上的限定词（已落到判据表与汇总表两处）

> **P0.3b = 2 条，只对当前 revision 有效。**
> 它**不含** `(1.5°, 3.0°]` 带内在 `27670b3`(00:37) 之前的历史违规 ——
> 该带 **P0.3b 分母内 8 条**（`corpus=='rotated'`），**同带另有 2 条 `anchor`**（`c06` −1.730、`c12` −2.090）
> **不在该分母里**，**合计 10 条**曾落在旧死区内（当前 **10/10 均已正确摆正**）。

**⚠ 缺口（同一天自查发现）：机器可读产物 `out/gate_P0_r2.json` 里没有这句。**
它的 `P0.3b.actual` 原文只有"违规 2 条……旋转夹具原始总数 78 条，需要摆正 67 条"，
**不带 revision 适用范围** —— 而下游更可能读 JSON，不读这份 md。

**处置（两条，都不改历史）：**

1. **不追溯改 `out/gate_P0_r2.json`。** 它是 r2 那一轮门禁的**原始输出记录**，
   **改它等于篡改裁决记录**（哪怕只加注释、不改 pass 值）。**记录的价值在于"当时输出了什么"。**
2. **r3 起，`P0.3b.actual` 必须自带适用范围**：
   `P0.3b=2` 是当前的残量，**不含 `(1.5°, 3.0°]` 在 `27670b3` 之前的历史违规**；
   **并同时写明本项只对判据声明节的那个 revision 有效。**
   **记为 r3 强制项。**

**这条缺口本身是同一形态的第四次显形**：**限定词写进了人读的地方，没写进机器读的地方** ——
两处"覆盖范围"不一致，而**只有一处会被自动消费**。

**为什么必须写**：否则下游会把"P0.3b = 2"读成"**摆正问题就剩这两条**"。
实际 P0 至少有三条腿：

| # | 腿 | 归谁 | 状态 |
|---|---|---|---|
| ① | 死区盲区（`|tilt| ≤ 3°` 完全不修正） | imaging | **已修**（`27670b3`，8/8 复算通过） |
| ② | 估计器 `unavailable` → 施加 0 | ml-porting | **未修**（r3 要修的） |
| ③ | 门禁当前测到的 | —— | **2 条**，即 ① 与 ② 收敛后的残量 |

**①③ 已闭合，②未闭合** —— 而判决里那个"2"只描述 ③。

## 24.5 判决未动

**P0.2 PASS、P0.3b FAIL = 2 条、r2 FAIL 10/12 MANUAL 1 —— 一个数都没改。**
本节加的是**适用域**，不是数值。**这是本项目"名字/数字声称的口径宽于它实际钉住的范围"的又一例**：
`P0.3b = 2` 这个数**没错**，但**它描述的 revision 范围没写在数旁边**。

---

# 勘误 25：**"不改记录"不等于"只能让 JSON 读者裸奔"** —— 加了一份只增不改的旁注

## 25.1 决定

主会话指出我漏了中间一档。**接受，已做**：新增 `out/GATE_P0_r2.caveats.json`（`out/GATE_*` 是我的领地），
**只增不改**，机器可读地承载本轮判决的**适用域与覆盖缺口**：

| id | 内容 |
|---|---|
| `C1-revision-scope` | P0.3b = 2 只对判据声明节那个 revision 有效；**不得读成"摆正问题只剩这两条"** |
| `C2-deadzone-band-10-not-8` | 旧死区吞掉的是 **10** 条（8 `rotated` + 2 `anchor`）；分母口径 `corpus=='rotated'` |
| `C3-no-criterion-covers-visible-slicing` | **没有任何判据覆盖"成片看起来被切一刀"** |
| `C4-alpha-holes-not-version-bound` | `P0_alpha_holes.json` 的指纹绑不住它量的 alpha；`c06_d-3` 从未被扫过 |

**`out/gate_P0_r2.json` 一个字节没动** —— 它是那一轮的**原始输出记录**，
**记录的价值在于"当时输出了什么"**。旁注与记录并列，读者同时读两者。

**决策依据（我原话的反面）**：我前一天刚写下"**加在会被消费的载体上，不是'我顺手在写'的那个**"，
然后**自己就只改了 md**。**如果下游只读 JSON，那么一个没人读的 `.md` 限定词等于没加。**

## 25.2 顺着它挖出的覆盖缺口（**这条比旁注本身重要**）

同一份 JSON 里嵌着 P0.5c 设备端自检的原始输出。**G4 的 2B.9「边界安全」** 判据原文：

```
最大越界比例=100.0%
图外非白像素（最差单张）=0 (阈值 0)        ← 这才是判据
setup0 越界比例=26.0% 缩小=false
setup1 越界比例=28.7% 缩小=false
setup3 越界比例=7.6%  缩小=true   说明=已按头顶锚点缩到越界 ≤ 45%
setup4 越界比例=31.0% 缩小=false
```

**⇒ 2B.9 只禁止"图外出现非底色像素"。越界本身、以及由此产生的斜切口，是它明确允许的** ——
越出部分按 alpha=0 填底色，**填的就是底色，判据当然为 0**。
**设计容忍上限 `maxOutOfBounds = 0.45`**（`lib/core/imaging/crop_geometry.dart:416`；
G4 自检也照此断言 `dev_repro_g4.dart:162` 的 `lessThanOrEqualTo(0.45)`）。

**而用户 `p1` 的越界是 15.46%**（`out/P0_compose_items.jsonl`，`cn_big_1inch`）
—— 远在容忍之内，**故不触发缩小**，于是成片上留下那道斜切口。

**⇒ 没有任何一条门禁判据覆盖"成片看起来被切了一刀"。**

**判据问的是"图外有没有非底色像素"，用户看的是"身体被斜着切断了"。**
**判据的范围窄于用户可见属性** —— 又是今天那个形态，**但这次不是谁的实现错，是判据本身没写这一条。**

## 25.3 处置：**显式写明"本门禁对此没有读数"**

已按 `native/` / `android/` 的同一处理方式，写进报告顶部「判据明确不覆盖的属性」块：

> **「成片看起来被切了一刀」没有任何判据覆盖 —— 本门禁对此没有读数。**

**「没扫」与「扫过、0 命中」在报告里长得一样，所以必须分开写** ——
**沉默比缺失更容易被当成"没问题"。**
一份只写着"2B.9 通过"的报告，会让人以为边界安全被管住了；**实际它管的是另一件事。**

**本门禁不断言该不该收紧。** 那是产品决定，可能要走 `docs/ACCEPTANCE.md`，
**而它只有主会话能改、且只在人工明确要求时改**（宪法 §7）。
**我只报事实：这道口子现在是敞开的，且在判据里看不见。**

## 25.4 顺带确认（主会话提到）

`2B.8` 里已有 `输入 roll=0.8°（死区内）→ 是否旋转=false` ——
**死区现在是设备端自检里被显式测到的行为，不再只是代码里的一个常数。** 这条比原来强。
