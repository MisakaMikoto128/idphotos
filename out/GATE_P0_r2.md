...
> 具体到 P0.3b：**`= 2` 不含 `(1.5°, 3.0°]` 那 8 条在 `27670b3`(2026-09-17 00:37, 死区 3°→1°) 之前的历史违规**
...
| 项 | 期望 | 实测 | 责任 agent |
|---|---|---|---|
| P0.3b | 真值 \|tilt\| > 1.5° 的 67 条夹具上 `unavailable` = 0 | **违规 2 条**：`c06_d-3`(truth −4.730°)、`c08_d-10`(truth −18.010°)，两条成片**未施加任何旋转**，仍歪着真值角度。**⚠ 此计数只对当前 revision 有效**（勘误 24）：不含 `(1.5°, 3.0°]` 的 8 条在 `27670b3`(00:37) 之前的历史违规 | ml-porting |
...
- 旋转夹具总数 **78**，`|真值| > 1.5` 即 P0.3b 分母 **67**（与判据表一致）；
...
> 它**不含** `(1.5°, 3.0°]` 这 8 条在 `27670b3`(00:37) 之前的历史违规
> —— 那段已被该 commit 修掉，故**在当前 revision 上不可测**。
