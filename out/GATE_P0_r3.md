...
### 本轮输入的一致性（读数绑在哪个版本上）
- HEAD = `5323695`；判 selfcheck 那几条（P0.5a）编译自**工作树**，不是这个 commit。
- `git status lib/` **干净**：本轮读数可绑定到 `5323695`。
...
- 3 份测量产出均可自证与当前树同源（受钉源码 103 个文件 = `lib/` 全部 `.dart` + `test/batch/` 全部 `.dart`/`.py`；代码摘要 -32be85b2dab13e3e）；标定靶：test/golden/src/g01.jpg 与内容钉一致（sha256=25b14d07b1375bc3…）；期间代码已变（0f66aabb3c08105f→-32be85b2dab13e3e）但标定靶未变，属预期
...
- 核了 3 份源产物：out/P0_output_residual.json=不一致、out/P0_compose_items.jsonl=不一致、out/P0_alpha_holes.json=不一致
  - `out/P0_output_residual.json`：记录 `249a56ec358f9f5a…`，盘上 `a80d66ee4e81262f…`
  - `out/P0_compose_items.jsonl`：记录 `aea8b3cc9ab95f75…`，盘上 `8c39d726caaf970a…`
  - `out/P0_alpha_holes.json`：记录 `67497012047ee087…`，盘上 `3ad3a37951061645…`
- 结论：**不相符 → 本项 FAIL**。逐条：
  - out/P0_alpha_holes.json：记录的摘要 67497012047ee087… ≠ 盘上现算的 3ad3a37951061645… —— **产物在 finalize 之后被改过**，叙述里的行值/条数描述的不是现在这份产物
  - out/P0_compose_items.jsonl：记录的摘要 aea8b3cc9ab95f75… ≠ 盘上现算的 8c39d726caaf970a… —— **产物在 finalize 之后被改过**，叙述里的行值/条数描述的不是现在这份产物
  - out/P0_output_residual.json：记录的摘要 249a56ec358f9f5a… ≠ 盘上现算的 a80d66ee4e81262f… —— **产物在 finalize 之后被改过**，叙述里的行值/条数描述的不是现在这份产物
...
- 实测：
  **不相符**：核了 3 份源产物：out/P0_output_residual.json=不一致、out/P0_compose_items.jsonl=不一致、out/P0_alpha_holes.json=不一致
  - out/P0_alpha_holes.json：记录的摘要 67497012047ee087… ≠ 盘上现算的 3ad3a37951061645… —— **产物在 finalize 之后被改过**，叙述里的行值/条数描述的不是现在这份产物
  - out/P0_compose_items.jsonl：记录的摘要 aea8b3cc9ab95f75… ≠ 盘上现算的 8c39d726caaf970a… —— **产物在 finalize 之后被改过**，叙述里的行值/条数描述的不是现在这份产物
  - out/P0_output_residual.json：记录的摘要 249a56ec358f9f5a… ≠ 盘上现算的 a80d66ee4e81262f… —— **产物在 finalize 之后被改过**，叙述里的行值/条数描述的不是现在这份产物
#### AC PASS
...
| EVID 派生块证据溯源：`gateInputs/rotationFailureBoundary/evidenceProvenance/sourceSha256` 记的摘要 == 盘上产物现算的摘要 | 逐份相符（**只证"叙述与产物一致"，不证"产物出自当前代码"**） | **不相符**：核了 3 份源产物：out/P0_output_residual.json=不一致、out/P0_compose_items.jsonl=不一致、out/P0_alpha_holes.json=不一致 | qa-batch（叙述与产物不同源；若产物被 finalize 之后重跑过，重跑 finalize 即可） |

### 回派指令
- ml-porting：P0.3a 未达标，详见上面各项「实测」。
- gatekeeper（本轮未重跑，无法判定是否退化；留待上游修好后补跑）：P0.5c 未达标，详见上面各项「实测」。
- qa-batch（叙述与产物不同源；若产物被 finalize 之后重跑过，重跑 finalize 即可）：EVID 未达标，详见上面各项「实测」。
...
**G2B-P0 第 3 轮：FAIL。通过 9 / 12 项，MANUAL 1 项。退出码 = 1（实测）。**

**本轮有效**：无 `roundInvalid`；三份被钉产出均**自证与当前树同源**（103 个文件逐条 `git hash-object` 比对，**0 处不一致**，指纹 `cd417a4d254ec1c2`）。

**P0.3b 已 PASS —— ml-porting 的修复有效。** 但 P0.3a 出现一条超线，**而这一条有一个必须由主会话裁定的问题**（见下）。

## 失败项

| 项 | 期望 | 实测 | 责任 agent |
|---|---|---|---|
| **P0.3a** | max\|成片残余\| ≤ 1.5° | **1.7534°**，超线 **1 条**：`c06_d+3` | ml-porting（**但见下，归属待裁定**） |
| **EVID** | 叙述摘要 == 盘上现算摘要 | **3 份全不一致**，产物在 finalize 之后被重跑过 | qa-batch（重跑 finalize 即可） |
| P0.5c | 原本通过的项重跑后仍 PASS | 未重跑，无读数 | gatekeeper |

## MANUAL 项（显式列出，**未**当作通过）

- **P0.5c**：判据要求"原本通过的项重跑后不退化"。生成层的条件是"上游 P0.3a/P0.3b 未修好时不重跑"——
  **P0.3b 已修好，P0.3a 未定**，故本轮仍未触发设备端重跑（一轮约 40 分钟）。
  **记 MANUAL，不计入通过数。** 若主会话裁定 P0.3a 属边界噪声 ⇒ 上游视为已修好 ⇒ **P0.5c 应补跑**。

## P0.3a：我判 FAIL，但**排除它适用贴线规则的那两条理由，本轮一条也不成立**

### 事实（逐条核过，均可复现）

1. **超线只有 1 条**：`c06_d+3` = **1.7534°**（眼线量具 `out/gate_P0_eyeline.json`）。
   **第二差是 `c12_d+3` = 1.4764°，低于阈值 0.024°。** 全 68 条 rotated 行里再无其它超线。
2. **第二支量具不复现这条超线**：SIFT（`out/gate_P0_sift_align.json`）给 `c06_d+3` = **1.2610°**，**在阈值之下**。
3. **余量 = 1.5 − 1.7534 = −0.2534°**；而两支量具**各自的自检误差**是
   **0.41°**（眼线，注入 ±3/±5/±10 六点）与 **0.500°**（刚体配准）。**余量 < 量具误差。**
4. 两支量具在**同一张成片**上相差 **0.49°**，与它们各自的自检误差同量级。

### 规则冲突（这是本轮的实质问题）

**主会话 2026-09-17 的贴线规则**（`tools/gate/gate_P0.dart:275`；同文见 `:892-900`）：

> 余量 < 量具误差的条目，**单支量具的 FAIL 不足以定罪** —— 必须第二支独立量具复现同一超线才计违规；
> 只有一支超线则记「边界噪声 / 证据不足」，**不消耗实现方修复轮次**。

**本轮 P0.3a 按这条规则的字面，正好落在它的射程内**：余量 −0.2534° < 量具误差 0.41°，且只有一支量具超线。

**但同一条规则明文把 P0.3a 排除在外**，理由是：

> **本规则不适用于 P0.3a**：它**超阈值 7 倍以上**且**两支量具独立复现**，是确定性违规。

**这两条理由本轮都不成立**：超阈值是 **1.17 倍**（不是 7 倍以上），第二支量具**未复现**（SIFT 1.2610° < 1.5°）。
**那段排除理由描述的是 r1 的状态**（P0.3a 当时 11.368°），**它没有被更新，于是从一条正确的判词变成了一个错误的豁免。**

**⇒ 我不自行适用该规则**：它明文排除 P0.3a，`tools/gate/**` 本轮冻结，且**阈值与判据只有主会话能动**。
**我按冻结判据记 FAIL**，并把冲突原样上交。**请先裁定，再决定要不要给 ml-porting 派活。**

### 机理（与上一条独立，无论怎么裁定都该记）

**三条最差的成片残余，恰好就是三条「引擎施加 0.0°」的样本** —— 真值都落在 1.0° 死区附近：
...
分组（眼线量具，rotated）：**小倾角**（\|真值\| ≤ 3.0）n=18 → max **1.7534**，中位 0.3710；
**大倾角**（> 3.0）n=50 → max **0.8725**，中位 0.3115。**小倾角组的最大值是大倾角组的 2.0 倍。**

**⇒ 死区把「不施加旋转」换成了「成片保留原倾角」，而那个倾角本身就是残余。**
死区 1.0°（`lib/core/imaging/crop_geometry.dart:125`，由 `27670b3` 从 3.0 收窄而来）
在 \|真值\| ≲ 1.3° 时**必然**留下最多约 1.3° 的成片倾角 —— **与 1.5° 判据之间本来就只有薄薄一层余量。**

**这不是本轮引入的回归**：r2 时这三条同样施加 0.0。**是死区大小与判据阈值之间的余量问题。**

## P0.3b：**PASS**（ml-porting 的修复有效）

| | 分母（`rotated` 且 \|真值\| > 1.5） | `unavailable` | SIFT 配准 |
|---|---|---|---|
| r2 | 67 | **2**（`c06_d-3`、`c08_d-10`） | 98 / 100，max \|施加角差\| 5.036° |
| **r3** | 67 | **0** | **99 / 100，max \|施加角差\| 0.077°** |

**差集**：110 行里只有 **5 行**的施加角或来源变化，全部是 `c06_d+3`、`c06_d-3` 与 `c08_d-10` 的三个规格。
**`unavailable` → 误差 > 1.5° 的样本：0 条**（逐条查过，不是只看计数）。

## 残余口径（要求 2）

**本判决里的每一个残余读数，都出自 gatekeeper 自己的量具**：
`out/gate_P0_eyeline.json`（88 条）与 `out/gate_P0_sift_align.json`（11 条），另 1 条 `derived_identity`。
**未引用 ml-porting 的「估计值 vs 真值」那三个数（0.31 / 0.19 / 0.48）作为残余。**

## 盲区（要求 1-3：量不出来的必须挂牌，不许静默通过也不许静默判失败）

- **`c08_d-10`：残余无法独立确认，记盲区。** 真值本身标注 `lowConfidence` ——
  `out/P0_truth.json` → `/anchors/8/note`：「暗光摄像头；**Haar 全程失败，仅 2 法可用且互差 1.69**，标记 lowConfidence」。
  **真值自身不确定度 ~1.7°，大于 1.5° 判据** ⇒ 没有任何独立路径能判它准不准。
  本轮它的施加角 = −17.8222（`pupil`），**不再是 `unavailable`**（方向必然更好：修前 0°、错约 18°）；
  但「准不准」**是公开盲区，按盲区记**。**本门禁对该样本的残余没有读数。**
- **`derived_identity` 1 条**：两条量具都测不出，用已验证恒等式 残余 = 真值 − 施加角 推出，**不是测出来的**。

## 覆盖缺口（本门禁对此没有读数）—— 要求 5

1. **`out/P0_alpha_holes.json` 不被任何判据读。** `tools/gate/gate_P0.dart` 里 `alpha_holes` 只出现**一处**
   （`:2776`），且是 `writeln` 的**字符串字面量**，不是判据 —— **它进不了 PASS/FAIL。**
   它量的是 `out/P0_alpha_scan/` 的 alpha；**该扫描本轮已随 qa-batch 重跑**
   （`manifest.jsonl` 09:04:31、产物 09:06:35），**故与本轮代码同源**。
   **但**：其 `provenance.inputProvenance` 记录了该产物历史上的同源缺口，且 `items` 的 87 条里只有 **21 条可评估**
   （详见 `out/GATE_P0_r2.caveats.json` 的 C4、C5）。
2. **「成片看起来被切了一刀」没有任何判据覆盖**（C3；`maxOutOfBounds = 0.45`，`p1` 越界 15.46% 不触发缩小）。
3. **`|真值| ≤ 1.5°` 的样本没有残余判据。** `kCoverageMinTruthDeg = 1.5`（`gate_P0.dart:180`）
   既是 P0.3b 的分母门槛（`:748`），又把小倾角样本挡在残余统计外 ——
   **而死区恰好就在这个区间里留下最大的成片倾角**（见上）。**这是一个结构性的盲区，不是偶然。**

## 脚本完整性

- 自动行报 `OK（本轮自报改动：19 个）`。**该行在自报文件上仍是盲的**（已知洞，主会话裁定 r3 之后再修）。
- **独立补偿控制**：`out/REVIEW_r3_start_baseline.txt`（HEAD `85b8403`，33 个文件）逐条比对当前盘面，
  **2 处差异，同源于同一个 gatekeeper 提交 `f676e19`**（`p0_eyeline.py` + `out/GATE_P0_selfchanges.txt`），
  已声明于 `out/GATE_P0_r2.md:966` 与 `out/GATE_P0_selfchanges.txt:8`。**判据、阈值、分母一律未动。**

## 防作弊巡查（条款 1–6）

| 条款 | 读数 | 判定 |
|---|---|---|
| 1 实现类 agent 未碰受保护路径 | 自 `baseline-p6p0-r2` 起，受保护路径只有 3 条改动，作者均为 gatekeeper（`f676e19`/`4e33340`） | 清白 |
| 2 gate 脚本 SHA256 | 26 个脚本全量重算，**仅 `p0_eyeline.py` 一处**，已声明 | 清白 |
| 3 `skip:` / 注释掉的断言 | 见生成层扫描表；`lib/` 0 命中 | 清白 |
| 4 `test/golden/` 文件数 | src=8 / ref=8，未减少 | 清白 |
| 5 空 catch | `lib/` 0 处 | 清白 |
| 6 报告数字 vs `out/gate_P0_r3.json` | `pass=false`、`9/12`、`manual=1`、退出码 1，逐项一致 | 清白 |

## 回派指令

1. **主会话（最优先）**：裁定 P0.3a 是否适用贴线规则。
   **该规则排除 P0.3a 所依据的两条理由（超阈值 7 倍以上、两支量具独立复现）本轮均不成立。**
   **在裁定之前请不要给 ml-porting 派 P0.3a 的活** —— 若裁定为边界噪声，改它等于白烧一轮。
2. **qa-batch**：EVID —— 三份产物在 finalize 之后被重跑过，**重跑 `test/batch/p0_finalize_v1.py` 即可**。
3. **imaging**：死区 1.0° 与小倾角成片残余的关系（机理见上）。**是否调整死区是产品/阈值决定，本门不禁。**
4. **ml-porting**：**P0.3b 已达标，本轮无待修项。** P0.3a 待主会话裁定。
...
| r3（预跑） | 作废（qa-batch 重测产物尚未生成） | 否 |
| **r3（本轮）** | **FAIL（9/12，P0.3a + EVID）** | **取决于上述裁定** |
