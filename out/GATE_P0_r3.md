...
> **【2026-09-17 行内更正标记，只加不改】** 这一小节与下面「轮次记账」的**记账口径已过期**。
> 现行冻结点是 **`d4fb782`**（主会话裁定），轮次账按**消耗**算、且**按实现方分账**。
> 逐条取代关系见本文件末尾的**补记 6**。下面两段原文**一字未删**，保留为 r3 当时的记录；
> **单读它们会读成"名额 = 轮次"，那是错的。**
...
**⚠ 上一行已由补记 6 取代（2026-09-17 主会话裁定）：**「3 次修复机会」**不是**"3 轮门禁运行"，
预算**按实现方分账**且**只在实际 FAIL 时消耗** —— r1 作废不消耗、r2 归 ml-porting 第 1 次、
r3 不消耗（本轮无效 + P0.3a 判边界噪声）。**"最后一轮" ≠ "预算用尽"。**
本轮按 `r3` 编号。
