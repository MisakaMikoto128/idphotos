...
**三条独立反例，逐条可复现：**
1. **零旋转也有洞**：`c08_upright` 真值 **0.0°**，洞 **0.6193** —— 比 −3.01°（0.1966）和 −5.01°（0.5277）那两张都大。直接否掉「洞只出现在被旋转过的输入上」。
2. **角与洞不单调**：−11.01° 那张是 **0.0**，夹在 0.5277 与 0.7549 中间。纯旋转机制给不出这种走法。
3. **【我补的第三条，比前两条更硬】同一源的 0° 原图与 0° 夹具不等**：c08 的**源照片** `C:/Users/liuyu/Pictures/Camera Roll/WIN_20230522_00_19_11_Pro.jpg` 在该文件自己的 `items` 里 `eyeBandZeroFrac = **0.0**`，而由**同一源、同样 0°** 产出的 `c08_upright.png` 是 **0.6193**。**同源、同角、结果相反** ⇒ 变的不只是旋转 —— **夹具流水线本身**（`out/P0_anchors/*.png` 的裁切/缩放/重编码）也在起作用。（严格说夹具是加工过的输入，"同内容"不等于"同输入"；但这恰恰是问题：该设计一次改了源身份、旋转、加工三件事。）

**设计问题**：对照是「**14 张真实照片 × 1 姿态**」对「**1 个源 × 7 姿态**」——**源身份与是否旋转完全混同**。c08 本就是 LOWCONF 源，「洞是 c08 这个源的抠图难点」与「洞由旋转诱导」在这份设计里**同样解释得通**，分不开。
