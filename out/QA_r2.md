...
1. **「零旋转」这一点两种读法都推不出原意**（team-lead 收紧了我先前的过硬说法：未核实 `c08_upright.png` 是"根本没转过"还是"转过但净角为 0"）：
   ...
   - **两种读法下，原句都推不出它想要的意思。**
2. **角与洞不单调（无条件成立）**：−11.01° 那张是 **0.0**，夹在 0.5277 与 0.7549 中间。纯"旋转量诱导"给不出这种走法。
3. **【我补的第三条，无条件成立且最硬】原图与其回正副本不等**：c08 的**源照片** `C:/Users/liuyu/Pictures/Camera Roll/WIN_20230522_00_19_11_Pro.jpg` 在该文件自己的 `items` 里 `eyeBandZeroFrac = **0.0**`，而由它**反向旋转回正**产出的 `c08_upright.png` 是 **0.6193**。**原始输入没有洞、回正副本反而有洞** —— 且由第 2 条知"转了多少度"不是判据 ⇒ 变量在**合成/重采样/重编码这条加工链**上，不在角度上。
...
**该改成的写法**：「洞出现在 **c08 家族**；**是否由旋转诱导，本设计分离不出**」—— 且由反例 3 可知，**至少有一部分不是旋转造成的**。
