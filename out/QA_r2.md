...
| 夹具 | 真值旋转 | `eyeBandZeroFrac` |
|---|---|---|
| `c08_d+10` | +1.99° | 0.0018 |
| `c08_d+5` | −3.01° | 0.1966 |
| `c08_d+3` | −5.01° | 0.5277 |
| **`c08_upright`** | **0.0°** | **0.6193** ← 零旋转却有洞 |
| `c08_d-3` | −11.01° | **0.0** ← 非单调 |
| `c08_d-5` | −13.01° | 0.7549 |
| `c08_d-10` | −18.01° | 0.9677 |

**三条独立反例，逐条可复现：**
1. **「零旋转」这一点两种读法都推不出原意**（team-lead 收紧了我先前的过硬说法：未核实 `c08_upright.png` 是"根本没转过"还是"转过但净角为 0"）：
   - **若它算"被旋转过的输入"** —— 它确实算：`P0_truth.json` 的 `uprightSynthetic` 条目自述「**由 c08 (真值 −8.01°) 按真值反向旋转回正**合成的竖直样本」，即对锚点施加了约 **+8.01°** 的旋转（`truth_apply_deg` 记 0.0，指的是**成品的名义倾角**，不是"没加工"）。那么 `answer` 那句话对**角度**没有任何约束力 —— 它对 0.0° 也成立，**推不出"旋转诱导"**。
   - **若它不算** —— 那它 0.6193 的洞就是**直接反例**。
   - **两种读法下，原句都推不出它想要的意思。**
...
**性质（比"数据打脸"更准的定性）**：该字段的**计数是现算的**（`14`、`4/7` 由 `len(originals)`、`sum(...)/len(fixtures)` 生成），但**结论句是写死的字符串**（`test/batch/p0_alpha_holes.py:158-163`）：无论 per-fixture 数怎么变，「洞只出现在被旋转过的输入上」「故这是旋转诱导的抠图失效」都会**原样输出**。而它**同一句里现算出的那 4 条命中，就包含 `c08_upright` 这张真值 0° 的夹具** —— 计数与结论在同一行里互相否定。**与 §4 第 1 项（`codeFingerprintOfSources` 写死 `None`）同型：硬编码叙述不随产物漂移。**
...
| 11 | `test/batch/p0_alpha_holes.py`（生成 `out/P0_alpha_holes.json` 的 `answer` 字段） | `answer` 断言「洞只出现在被旋转过的输入上」，被其**自身数据**否掉：`c08_upright` 真值 0.0° 却有 0.6193 的洞；−11.01° 那张是 0.0（非单调）；**同源的 0° 原图 0.0 而其 0° 夹具 0.6193**。应改成「洞出现在 c08 家族；是否由旋转诱导，本设计分离不出」。**后置到 r3 判完之后**（改它要重生成被钉产出）。见 §3.5。 | qa-batch |
