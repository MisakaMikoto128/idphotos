...
对 `out/P0_output_residual.json` 里**全部 99 条** `m3_haar_eyeline` 记录做验证：每条读数的 `left`/`right` 坐标**都是整数**，且

    deg == atan2(right.y - left.y, right.x - left.x)   —— 99/99 逐条成立，0 例外

即该量具的分辨率**不是连续值**，而是由眼距 `dx` 决定的**离散栅格**：`quantum = atan(1/dx)`。本批成片眼距 ≈90 px ⇒ **quantum ≈ 0.637°**。

这解释了两件先前看起来矛盾的事：

1. **「跨样本逐位相同」不是串样本**：`0.6365935759634865` 在 5 处出现（`out/P0_output_residual.json`），等于 `atan(±1/90)`。可取值只有那么几百个，**同值碰撞是量化的必然**（team-lead 的 `c04` 与 gatekeeper 的 `c05_upright` 撞的是同一个栅格点，不是同一张图）。
...
**佐证强度的边界（受 §2.2 约束）**：本节把 Haar 眼线与瞳孔估计器当作**跨方法佐证**是成立的，**但仅因为 `c06_d-3` 的 −4.051° 远高于 Haar 量具的分辨率**（≈0.637°，即约 **6.4 个量化步长**）。这条佐证**不可外推**：在近零残余区间（|tilt| ≲ 1.3°）该量具只能输出 0、±0.637、±1.273…，既不能佐证也不能反驳。即「跨方法一致」在**大角度可用、在阈值(1.5°)附近不可用** —— 后者的现成反例就是 §2.2 里 gatekeeper 在 P0.2 上报的那个数：它恰好等于**一个量化步长**。
