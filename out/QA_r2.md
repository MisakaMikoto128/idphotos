...
- **c08 至此的第 4 条独立信号**（前三条见 §4b/§4c）：① 眼带 alpha 空洞全在 c08 家族；② P0.3b 违规 `c08_d-10`；③ `ds_portrait_37` 整帧估角 dropout；④ 本轮 m1 在 c08 成片上失准：**3 张**有免眼读数确认（`c08_d+5` / `c08_d+10` / `c08_d-5`），另 **2 张**（`c08_d+3` / `c08_d-10`）无免眼读数、按 §2.5 归**"未知"**。四条**同指该 session 的成片/眼区对多种方法都不友好**。
...
**影响面**：gatekeeper 曾以本 `answer` 为**推理前提**论证「`c08_d-10` 的洞是夹具伪影」，该前提已被其自身数据否掉；其结论改由**更强**的理由支撑 —— **空洞与 roll 决策通路不相交**（roll 在原始输入上做：`yunet_decoder.dart:244-247`、`matting_worker.dart:399-411`；空洞在抠图输出里）。**裁定不变：`c08_d-10` 计入，P0.3b = 2 条。** 另注：**数据本身没问题**（`perFixture` 与 `items` 都是真的），错的是 `answer` 这个**结论字段**；§3.3「只有 `P0_alpha_holes.json` 是可用的洞信号」**仍然成立**。
