...
**引擎自报一列值得单说**：10 条**全部** `rollSource=pupil` + `straightenDeg=0.0`。即引擎对这 10 条**一次也没施加旋转**（正确：它们的**名义**倾角落在死区 `|roll|≤1.5°`），也**从未自报失败**。注意 `c08_upright`：引擎报 `pupil`，而 qa 量具在同一张成片上 `no_pair`（无法测量）——**「引擎自报成功」与「测量侧可测」是两件事**，`rollSource` 单独看不构成可用性证明。`P0.2_allSamplesReturnedPupil`（`straight 1/1` + `uprightSynthetic 9/9`）与此一致。

> **`*_upright` 的读法（team-lead 提出的通用规则；本文已按此回扫）**
> `out/P0_truth.json` 的 `uprightSynthetic` **9 条全是回正副本**（`p1 / c01 / c03 / c04 / c05 / c06 / c08 / c10 / c12 _upright`），每条的 `note` 都是「由 X (真值 −k°) 按真值**反向旋转回正**合成」。
> ⇒ **`*_upright` 描述的是「结果朝向」，不是「加工历史」；这一族里没有任何「没被旋转过」的样本。**
> ⇒ 本文凡出现"未旋转 / 没旋转"，一律指 **引擎未施加旋转**（`straightenDeg=0`），**不是**"该输入没被旋转过"。前者是引擎行为（可核 `straightenDeg`），后者由夹具合成方式决定（`note`）。
...
- **m3 还会污染可信度标签**（gatekeeper 指出的机制，影响面我已全量核实）：m3 的越界值撑大 `methodSpreadDeg`，而 `p0_output_residual.py:320` 用 `spread > 1.5°` 把行降级为 `low_confidence`。全量统计：23 条 `low_confidence` 中有 **7 条**（`c11_d-10`、`c12`、`c12_d+3`、`c12_d+5`、`c12_d-10`、`c12_d-5`、`p2_d-3`）是**因 m3 独自越界、其余方法彼此在 1.0° 以内**而降级的 —— 即用坏列打了自己的好列。**但当前损害止于标签**：① `:328` 把 `low_confidence` 照常计分（不排除）；② `qa_spread` 被 `gate_P0.dart:512` 收进样本表，**全仓没有任何判据读它**；③ 门禁只对 `qa_status ∈ {reliability_mismatch, unmeasured}` 声明「不采信」（`gate_P0.dart:2709`），`low_confidence` 不在其中。故本轮**无假排除、无假不采信**，缺陷是**标签失真 + 给未来消费者留了一个陷阱列**。
- **同类且更严重的一列：`yunet_eyeline_deg`**（`p0_output_residual.py:136` 记录）。gatekeeper 勘误 5 用免眼 SIFT 路径量得它与真值差 **>1.5° 的达 74/98 条**（中位 2.50°、最大 7.04°）。它在我的产物里**无人消费**，但同样**不得作判据**；r3 应显式标注为诊断量。
...
- **未验证的假设（team-lead 提出，只记一笔；不作待查缺陷、不排入 r3）**：这 8 条配对失败**集中在 c02 族、c07 族、p2 族**，而这几族恰好也是**裁剪缺陷**咬得最狠的（紧取景、成片把头部放大/切到边）；尤其 `p2` 检出眼宽 **39.5 / 期望 66.4**，只剩六成，形态上更像**取到了眉/鼻孔/眼镜框**这类异物，而不是"配对到另一只眼"。**假设**：m3 的配对失败可能是**成片构图**的下游症状（头部占比过大、眼睛贴近边缘时 Haar 双眼级联更易配错）。若成立，imaging 修好裁剪后这几条会自己消失。**价值在分岔诊断**：修完裁剪后若这 8 条**依旧**配错，那就是 m3 自身的问题，诊断方向完全不同。

**B. `c03_upright` 的 2.66° 摆动 —— qa 量具自身的脆弱性数据点，非判据输入。**

同一内容、只差裁切/缩放，qa 量具的瞳孔法（`m1_pupil`）在**夹具侧读 2.406**、在**成片侧读 −0.2527**（该条 **引擎未施加旋转**，`straightenDeg = 0.0` ⇒ 两侧差异**不在引擎的施加角上**），**摆动 2.66°**。**这不是待查缺陷**：`tools/gate/` 对 `measuredTiltDeg` 零命中，它推不动任何判决。保留在此，作为「瞳孔族在这类图上不稳」的**第三条独立信号**（与 §4c 同向），供 r3 参考。

**C. m1 自身的 6 条排除行 —— 我方量具在 c08/p2 **成片**上的失效（已被本报告自身的可信度闸拦下）。**

- **事实**：`out/P0_output_residual.json` 的 `summary.unreliableMeasurement` = 6 个不同 id：`c08_d+10 / c08_d+3 / c08_d+5 / c08_d-10 / c08_d-5 / p2_d-10`，`end_to_end_status` 全为 `reliability_mismatch`，`corpus` 全为 `rotated`（该清单按 id 去重；本案各 id 在盘上取的都是 **`cn_big_1inch`** 行）。它们被排除出 `scored`（rotated **78 → 72**）。
- **判据**：`p0_output_residual.py:309-319` —— `truthConsistencyDeg = rigid_residual_deg − truthTiltDeg`，`|·| > 2°` 即判「测量落到异物上，本项残余不可信」。
- **与 gatekeeper §5.3 的观察项同源**：其报的「qa 主用 `m1_pupil` 与免眼 SIFT 差 >1.5° 的 4 条」（`p2_d-10 −17.94` / `c08_d+5 +5.75` / `c08_d+10 −3.44` / `c08_d-5 +2.03`，**四条均 `cn_big_1inch`**）**全部落在这 6 条之内**。即两条**互不相同**的判据（我的「与真值一致性」、门禁的「与免眼路径一致性」）指向**同一批行** —— 这是**提高**而非降低「错的是 m1」的置信度。
- **机制（定位我方量具，不是指控引擎）**：这 6 条里 **5 条**引擎自报 `straightenDeg` 与真值相差 **≤0.42°**（0.033 / 0.159 / 0.181 / 0.218 / 0.417；**这 5 条均为 `cn_big_1inch` 行**），即**引擎的滚转估计是对的、成片本应基本水平** —— 故这一组的大角度读数不可能来自"引擎没摆正"。其中 **4 条**（`p2_d-10` / `c08_d+5` / `c08_d+10` / `c08_d-5`，均 `cn_big_1inch`）有免眼 SIFT 独立读数、确认**错的是 m1**；第 5 条 `c08_d+3` 因免眼路径无读数（`weak_alignment(inl=63)`），**按 §2.5 归"未知"，不在此归类为"量错"。**原因与 §4c 同：**m1 读的是成片，成片在抠图下游**；c08 这一 session 的眼带被 alpha 空洞打掉（§4b①），成片眼区已非原貌，瞳孔法在那上面会锁到异物。引擎读**原图**灰度，故不受影响。
- **一个盲点（r3 修）**：第 6 条 `c08_d-10` 方向相反 —— 真值 **−18.01°**、引擎 `straightenDeg = 0.000`、`rollSource = unavailable`，即**引擎确实没摆正**（此项由**引擎自报 + 真值**判定，**不依赖 m1 的读数**）；而该行仍被同一闸判成 `reliability_mismatch` 排除。**该闸区分不了「m1 错」与「引擎错」—— 它只有一路信息（量具与真值之差）。** 本轮未造成漏判：这条的失败由 P0.3b（`rollSource` 不可用）**独立捕获**，故 P0.3b 违规仍为 2 条、判定不动。但若某轮只靠残余判据，一条 ~18° 的真实失败会被当成「测量不可信」消掉。**门禁侧有兜底**：`gate_P0.dart:2709` 读到 `qa_status ∈ {reliability_mismatch, unmeasured}` 时会写明「已被 qa-batch 自己声明不可靠，不采信」并**改用 SIFT 读数** —— 故本轮的代价不是漏掉失败，而是**丢掉一条本可用的读数**。
- **【订正：我自己引错了行】** 同一个 id 在**三张 spec 下是三行、读数不同**：`c08_d-10` 在 `cn_1inch` = `no_pair`（unmeasured）/ **`cn_big_1inch` = +3.852** / `visa_us` = −14.146（引擎行为三行相同：`straightenDeg=0`、`rollSource=unavailable`）。**门禁规格是 `cn_big_1inch`**（`gate_P0.dart:45`；`gate_P0.dart:3392` 按 `specId` 过滤）。我先前在本节引的 **−14.146 是 `visa_us` 那一行，不是门禁规格行**，据此写的「m1 读 −14.146、方向正确」**一句就此撤回**：在门禁规格上 m1 读 **+3.852**，**符号都翻**。**引残余数字一律带 `specId`。**
- **`m1` 的可靠性随规格变（零成本得到的一条独立证据，指认 m1 是脆弱的一环）**：同一个夹具 `c08_d-10`、**同样的引擎行为**（三行都是 `straightenDeg=0`、`rollSource=unavailable`、真值 −18.01），m1 在三张 spec 上读出三个不同的结论 —— `cn_1inch` `no_pair` / **`cn_big_1inch` +3.852（符号都翻）** / `visa_us` −14.146（大致对）。**同一输入、同一失败，仅换输出规格就读数反向** ⇒ 出问题的不是被量对象，是 m1 的选点。
- **对判据的影响**：P0.3a / P0.3b 的分母与违规数**不变**；§2 的 0.290 / 0.575 / 0.145 等数均来自**未被排除**的行。
- **c08 至此的第 4 条独立信号**（前三条见 §4b/§4c）：① 眼带 alpha 空洞全在 c08 家族；② P0.3b 违规 `c08_d-10`；③ `ds_portrait_37` 整帧估角 dropout；④ 本轮 m1 在 c08 成片上失准：**3 张**有免眼读数确认（`c08_d+5` / `c08_d+10` / `c08_d-5`），另 **2 张**（`c08_d+3` / `c08_d-10`）无免眼读数、按 §2.5 归**"未知"**。①的洞数据**未绑定代码状态，见 §3.6**；④来自 `P0_output_residual.json`，是绑定的。四条**同指该 session 的成片/眼区对多种方法都不友好**。
- **r3 动作**：排除判定须与 `rollSource` 解耦 —— `rollSource = unavailable`（引擎未动作）的行**不得**因 `truthConsistencyDeg` 过大而被静默排除，应单列。

### 2.5 主用列 `m1_pupil` 在**大角度**上的失效，以及**自报质量量为何发现不了**

team-lead 用免眼 SIFT（`out/gate_P0_sift_align.json` 的 `residual_deg`，不碰眼点、不碰 alpha）裁 `cn_big_1inch`（n=100，`n_ok` 98）中 `|primary_tilt_deg| > 1.5` 的 7 条。**我逐条对过 `out/P0_output_residual.json` 与 `out/gate_P0_sift_align.json` 两个产物，读数一致**，表格照录：

| 样本 | `m1_pupil`（qa 主用） | `m3` | 免眼 SIFT 残余 | 判定 |
|---|---|---|---|---|
| `p2_d-10` | **−17.937** | −0.306 | **−0.068** | m1 错 **17.9°** |
| `c08_d+5` | +5.755 | —（`no_pair`） | **−0.170** | m1 错 5.9° |
| `c08_d+10` | −3.445 | — | **−0.179** | m1 错 3.3° |
| `c08_d-5` | +2.028 | —（`no_pair`） | **−0.209** | m1 错 2.2° |
| `c06_d-3` | −4.825 | −4.354 | **−4.742** | **三台一致 —— 真歪，m1 是对的，不算量错** |
| `c08_d+3` | −11.349 | —（`no_pair`） | **无读数**（`weak_alignment(inl=63)`） | **未知 —— 不许归类** |
| `c08_d-10` | **+3.852**（`cn_big_1inch`） | —（`no_pair`） | **无读数**（`weak_alignment(inl=67)`） | **未知 —— 不许归类** |

**结论一**：`|d| ≥ 3°` 时 qa 主用列在 **c08 族 5 条 + `p2_d-10`** 上失效；`c06_d-3` 必须排除在"量错"之外。两条无读数的**只能写"未知"**，不得并入任一侧。

**结论二（更要紧，根因已查到源码行）**：`p2_d-10` 的自报质量量**全部正常** —— `pairCost 0.357`、`eyedist 91.3`、`seed: yunet` —— 却错了 17.9°。**`pairCost` / `eyedist` 不构成充分质量闸。**

而且 `eyedist` 这个字段**结构上就查不出这种错**：`p0_output_residual.py:91` 把它算成 `hypot(ir − il)`，其中 `il, ir` 是**种子点（YuNet）**；`:110` 原样写进 m1 记录。**它是输入眼距的回显，不是 m1 自己选出的两点的距离。** 全量验证：**`m1_pupil.eyedist` 与 `yunet_eyedist` 108/108 逐位相同**；而它与 m1 **自己** `left`/`right` 两点距离**差 >2 px 的有 74/108 条**，最坏 `c08_d+3` 报 91.7、实际两点距 **139.7**（差 48）。**一个按"瞳孔距离异常"设计的检查量的是输入，所以对输出的错配对完全无感 —— 该有的检查（两点距离）从未被计算过。**
...
`P0_alpha_holes.json`：total 87，assessable 21，眼带空洞全部落在 c08 家族 4 条（`c08_d-10` 0.9677 / `c08_d-5` 0.7549 / `c08_upright` 0.6193 / `c08_d+3` 0.5277）。**这与 P0.3b 的 `c08_d-10` 违规是同一张图上的两个独立信号**，互相印证。**⚠ 本段全部数字来自未绑定代码状态的产物（见 §3.6）。**
...
3. **另一条独立理由（gatekeeper 提供）**：`outOfBoundsFraction > 0` 在 110 行里占 77 行（70%，最大 43.5%），而越界区域**按设计填 alpha=0** ⇒ 成片端 alpha=0 无法区分「设计留白」与「抠图失败」。结论：**只有 `P0_alpha_holes.json`（干净原图侧）是可用的洞信号** —— 但该产物**未绑定代码状态**，见 §3.6。
4. 两条理由都属**咨询性质**：目前没有任何判据读 `alphaHoleHint`。

### 3.5 `P0_alpha_holes.json` 的 `answer` 说了它自己的数据不支持的话（team-lead 发现，我已逐项核验）

`answer` 原文：「**否。未旋转的真实照片一张都没有洞**（Pictures 可评估人像 14 张，eyeBandZeroFrac 全部 = 0.0，含两张 Camera Roll 原图）。洞只出现在**被旋转过的输入**上：c08 夹具家族 4/7 命中。故这是**旋转诱导的抠图失效**。」

把该文件**自己的** `controlComparison.rotatedFixtures.perFixture` 配上真值角（`out/gate_P0_sift_align.json` / `truthTiltDeg`）：

| 夹具 | 名义倾角（`truthTiltDeg`） | `eyeBandZeroFrac` |
|---|---|---|
| `c08_d+10` | +1.99° | 0.0018 |
| `c08_d+5` | −3.01° | 0.1966 |
| `c08_d+3` | −5.01° | 0.5277 |
| **`c08_upright`** | **名义 0.0°**（**回正副本，非未旋转**） | **0.6193** ← 名义 0° 却有洞 |
| `c08_d-3` | −11.01° | **0.0** ← 非单调 |
| `c08_d-5` | −13.01° | 0.7549 |
| `c08_d-10` | −18.01° | 0.9677 |

**三条独立反例，逐条可复现：**
1. **`c08_upright` 是回正副本，"未旋转"这个前提本身不成立**（按 `note` 核实来路，不再是"两种读法"）：`P0_truth.json` 的 `uprightSynthetic` 条目自述「**由 c08 (真值 −8.01°) 按真值反向旋转回正**合成的竖直样本」⇒ 它**转过约 +8.01°**；`truth_apply_deg: 0.0` 指的是**成品的名义倾角**，**不是"没加工"**。因此 `answer` 那句"洞只出现在被旋转过的输入上"**对角度没有任何约束力**（名义 0° 也满足它），**推不出"旋转诱导"**。（一般规则：`*_upright` 9 条**全是回正副本**，见 §2.1 的读法说明。）
2. **洞占比与角度不单调（无条件成立，且是本节唯一能下的结论）**：+1.99° → 0.0018、−3.01° → 0.1966、−5.01° → 0.5277、名义 0° → 0.6193、**−11.01° → 0.0**、−13.01° → 0.7549、−18.01° → 0.9677 ⇒ **旋转量不是驱动**。
3. **原图与其回正副本不等（观察成立，但它什么都分离不出）**：c08 的**源照片** `C:/Users/liuyu/Pictures/Camera Roll/WIN_20230522_00_19_11_Pro.jpg` 在该文件自己的 `items` 里 `eyeBandZeroFrac = **0.0**`，而由它产出的 `c08_upright.png` 是 **0.6193**。**注意这一对同时差在两件事上**：是否经过夹具加工（裁切/缩放/重编码）**与**是否旋转（该副本是**反向旋转约 +8.01° 回正**合成的）—— **所以它排除不了角度**。它只能说明"这一对输入不同、结果不同"，**不指向成因**。

**落点只有一句（team-lead 收窄；我先前写成"变量在加工链上、不在角度上"，是又给一个没分离出的变量归因，已撤回）**：**旋转量不是驱动**；**驱动它的到底是哪一步，本设计分不出来** ⇒ **成因未知**。"量"与"有无"是两个命题 —— 我证掉的是前者，**"有无旋转"没有被证掉**。

**设计问题**：对照是「**14 张真实照片 × 1 姿态**」对「**1 个源 × 7 姿态**」——**源身份与是否旋转完全混同**。c08 本就是 LOWCONF 源，「洞是 c08 这个源的抠图难点」与「洞由旋转诱导」在这份设计里**同样解释得通**，分不开。**而 c08 的源身份已查明并核验**：`P0_truth.json` 的锚点 `c08` 指向 `C:/Users/liuyu/Pictures/Camera Roll/WIN_20230522_00_19_11_Pro.jpg` —— 即**金标集的 g08**（`test/golden/mapping.json` 中 `source_path` 一致、`source_sha256 = f99d2fb4c309c964…` 与盘上文件逐位相符；note「**网络摄像头暗光室内自拍，杂乱背景，非正规证件照场景**」；`foreground_ratio` **0.2103**，为 8 张金标中最低）。**即 c08 家族的 7 张全部出自金标里最难的那张源。** 「MODNet 在这张源上本来就容易打洞」是一个**比"旋转诱导"更朴素、且与全部数据一致**的解释，本设计分不开它俩。

**该改成的写法**：「洞出现在 **c08 家族**；**是否由旋转诱导，本设计分离不出**」—— 可额外写「**旋转量不是驱动**」（不单调已证），但**驱动它的究竟是哪一步同样分不出**；**落点是"成因未知"，不是"在加工链上"**。

**另：真值文件这一层是干净的，不要一起否掉。** `out/P0_truth.json` 的 `/gateInputs/notScorableClassification/items/6` 已把 `c08_upright` 定为 `{class: "抠图失效", eyeBandZeroFrac: 0.6193, reason: "双眼区 alpha=0 占比 0.619 > 0.35，瞳孔被挖掉，成片眼线不可测"}` —— **这个定性是对的，错的只是成因归属**，而那句归属在该文件之外（`p0_alpha_holes.py` 的 `answer`）。

**性质（比"数据打脸"更准的定性）**：该字段的**计数是现算的**（`14`、`4/7` 由 `len(originals)`、`sum(...)/len(fixtures)` 生成），但**结论句是写死的字符串**（`test/batch/p0_alpha_holes.py:158-163`）：无论 per-fixture 数怎么变，「洞只出现在被旋转过的输入上」「故这是旋转诱导的抠图失效」都会**原样输出**。而它**同一句里现算出的那 4 条命中，就包含 `c08_upright` 这张名义 0° 的**回正副本**夹具** —— 计数与结论在同一行里互相否定。**与 §4 第 1 项（`codeFingerprintOfSources` 写死 `None`）同型：硬编码叙述不随产物漂移。**

**处置（与 `eyedist` 同口径）**：`out/P0_alpha_holes.json` **r3 期间不许重生成**（在指纹域内，牵动三份被钉产出、有 `roundInvalid` 风险）。**写进报告、记成 r3 后队列项，现在一行不动。**

**影响面**：gatekeeper 曾以本 `answer` 为**推理前提**论证「`c08_d-10` 的洞是夹具伪影」，该前提已被其自身数据否掉；其结论改由**更强**的理由支撑 —— **空洞与 roll 决策通路不相交**（roll 在原始输入上做：`yunet_decoder.dart:244-247`、`matting_worker.dart:399-411`；空洞在抠图输出里）。**裁定不变：`c08_d-10` 计入，P0.3b = 2 条。** 另注：**数据本身没问题**（`perFixture` 与 `items` 都是真的），错的是 `answer` 这个**结论字段**；§3.3「只有 `P0_alpha_holes.json` 是可用的洞信号」**仍然成立**（但该产物**未绑定代码状态**，见 §3.6）。

### 3.6 `P0_alpha_holes.json` 记的指纹钉的是**包装**，不是它量的那份数据（team-lead 发现，我已核验）

- **事实（逐条在盘上核过）**：
  - 该文件是 `tools/gate/provenance.dart:369` 点名的**三份被钉测量产出之一**（第 ⑤ 条对它核 `provenance.codeFingerprint`）。
  - 但它量的是 `out/P0_alpha_scan/` 里**更早一次扫描**的 alpha。该扫描的 `out/P0_alpha_scan/manifest.jsonl` **87 行，字段只有 `err / outcome / path / zh`** —— `fingerprint` / `codeFingerprint` / `fnv1a64` / `sha256` / `hash` **一个都没有**，`0f66aabb` 也不在其中。
  - 该目录 **36 个文件**的 mtime 跨度 **03:45:04 → 07:40:39**（近四小时），期间 `lib/` 一直在变（瞳孔估计器当天多次改动）。
- **后果**：第 ⑤ 条对这份产出**核得过**，但它只证明「`p0_alpha_holes.py` 跑的时候代码没变」，**证不了那 87 份 alpha 出自哪份代码**。**钉住的是包装，不是内容。**
- **该文件自己承认了这一点**：`p0_alpha_holes.py:151-154` 的 `inputProvenance` 明写「那次扫描**没有记录自己的代码指纹**……不能证明那些 alpha 出自同一份代码」。**但这句承认也是字面量** —— 哪天扫描真的记了指纹，它**照样打印**（与 §3.5 的 `answer`、§4 第 1 项的 `codeFingerprintOfSources` 同型）。
- **影响的是"支撑面"，不是判据**：本报告里凡引用洞数据的陈述 —— 「c08 家族有洞」「14 张原图全 0.0」「只有 `P0_alpha_holes.json` 是可用的洞信号」—— **都是对一份未绑定代码状态的数据的陈述**。数据本身可能是对的（07:40 那次扫描的输入是干净原图/夹具，与 roll 通路不相交；裁定 `c08_d-10` 时也没用它），**但它不能被称为「本轮代码的读数」**。
- **处置**：r3 期间 `p0_alpha_holes.py` 与 `out/P0_alpha_holes.json` **一行不动**；队列项 12。
...
| 11 | `test/batch/p0_alpha_holes.py`（生成 `out/P0_alpha_holes.json` 的 `answer` 字段） | `answer` 断言「洞只出现在被旋转过的输入上」，被其**自身数据**否掉：`c08_upright` 是**回正副本**（转过约 +8.01°）、**名义**倾角 0.0°，却有 0.6193 的洞；−11.01° 那张是 0.0（非单调）；**同源的 0° 原图 0.0 而其名义 0° 夹具 0.6193**。应改成「洞出现在 c08 家族；是否由旋转诱导，本设计分离不出」。**后置到 r3 判完之后**（改它要重生成被钉产出）。见 §3.5。 | qa-batch |
| 12 | `test/batch/p0_alpha_scan_test.dart` + `p0_alpha_holes.py` | `out/P0_alpha_scan/manifest.jsonl`（87 行）**无任何指纹字段**，而 `out/P0_alpha_holes.json` 是 `provenance.dart:369` 的**被钉产出** ⇒ 第 ⑤ 条核的是**包装不是内容**（只证分析脚本跑时代码没变，证不了那 87 份 alpha 的来路；扫描 mtime 跨 03:45→07:40）。修法：让**扫描自己记指纹**，或让扫描与 `p0_alpha_holes.py` 在**同批、同一指纹**下产出；并把 `p0_alpha_holes.py:151-154` 那句**字面量**警告改成由实际字段驱动。**判据：凡被钉的产出，其钉必须覆盖它真正量的那份数据，而不是包住它的那层脚本。** 后置到 r3 之后。见 §3.6。 | qa-batch |
| 13 | `test/batch/build_golden.py:4`（docstring 首句） | 首句称黄金集是「**8 张最标准正面清晰人像**」，而**同一 docstring 的 `:8-12` 已准确列举**其构成（含「1 张真实场景网络摄像头照片（暗光室内、杂乱背景）」= **g08**，`foreground_ratio` **0.2103**，8 张中最低）。**摘要句比它下面两行的明细声称得更干净** —— 与 §3.5 的 `answer`、§4 第 1 项同型。**注：`test/golden/` 内无需改**（`mapping.json` 逐张 `note` 准确；`docs/ACCEPTANCE.md:19` 只写「≥ 8 张人像 JPEG」，无过度声称）。修法：首句与明细对齐（如「8 张人像，**刻意覆盖**证件照 / 复杂背景 / 暗光自拍等难度」）。**后置到 r3 之后**（`test/batch/` 在指纹域内）。 | qa-batch |
...
- **与锚点 c08 是同一次拍摄的相邻帧**：`..._00_19_21_Pro.jpg`（1272×716）与 **c08** = `..._00_19_11_Pro.jpg`（1280×720）相隔 10 秒（00:19:11 / 00:19:21）。至此 c08 这一被摄者/session 上汇集了三条读数：① `P0_alpha_holes` 的 4 条眼带空洞**全部**在 c08 家族（**该产物未绑定代码状态，见 §3.6**）；② P0.3b 违规之一 `c08_d-10`；③ 本条整帧估角 dropout。①与②可能同根因，但读数来自独立测量路径（alpha 通道扫描 vs 滚转残余）。综合指向**被摄者/session 特异**，而非「旋转夹具伪影」。
