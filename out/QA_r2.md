...
- **一个盲点（r3 修）**：第 6 条 `c08_d-10` 方向相反 —— 真值 **−18.01°**、引擎 `straightenDeg = 0.000`、`rollSource = unavailable`，即**引擎确实没摆正**；此时 m1 读 **−14.146**、**方向正确**，却被同一闸判成 `reliability_mismatch` 而排除。**该闸区分不了「m1 错」与「引擎错且 m1 大致对」。** 本轮未造成漏判：这条的失败由 P0.3b（`rollSource` 不可用）**独立捕获**，故 P0.3b 违规仍为 2 条、判定不动。但若某轮只靠残余判据，一条 ~14° 的真实失败会被当成「测量不可信」消掉。**不过门禁侧有兜底**：`gate_P0.dart:2709` 读到 `qa_status ∈ {reliability_mismatch, unmeasured}` 时会写明「已被 qa-batch 自己声明不可靠，不采信」并**改用 SIFT 读数**，故本轮的代价不是漏掉失败，而是**丢掉一条本可用的读数**。
