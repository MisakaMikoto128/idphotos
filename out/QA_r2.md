...
- **事实**：`out/P0_output_residual.json` 的 `summary.unreliableMeasurement` = 6 个不同 id：`c08_d+10 / c08_d+3 / c08_d+5 / c08_d-10 / c08_d-5 / p2_d-10`，`end_to_end_status` 全为 `reliability_mismatch`，`corpus` 全为 `rotated`。它们被排除出 `scored`（rotated **78 → 72**）。
- **判据**：`p0_output_residual.py:309-319` —— `truthConsistencyDeg = rigid_residual_deg − truthTiltDeg`，`|·| > 2°` 即判「测量落到异物上，本项残余不可信」。
- **与 gatekeeper §5.3 的观察项同源**：其报的「qa 主用 `m1_pupil` 与免眼 SIFT 差 >1.5° 的 4 条」（`p2_d-10 −17.94` / `c08_d+5 +5.75` / `c08_d+10 −3.44` / `c08_d-5 +2.03`）**全部落在这 6 条之内**。即两条**互不相同**的判据（我的「与真值一致性」、门禁的「与免眼路径一致性」）指向**同一批行** —— 这是**提高**而非降低「错的是 m1」的置信度。
- **机制（定位我方量具，不是指控引擎）**：这 6 条里 **5 条**引擎自报 `straightenDeg` 与真值相差 **≤0.42°**（0.033 / 0.159 / 0.181 / 0.218 / 0.417），即**引擎的滚转估计是对的、成片本应基本水平**；错的是**我 m1 在成片上的读数**（偏 3.4–17.9°）。原因与 §4c 同：**m1 读的是成片，成片在抠图下游**；c08 这一 session 的眼带被 alpha 空洞打掉（§4b①），成片眼区已非原貌，瞳孔法在那上面会锁到异物。引擎读**原图**灰度，故不受影响。
...
- **c08 至此的第 4 条独立信号**（前三条见 §4b/§4c）：① 眼带 alpha 空洞全在 c08 家族；② P0.3b 违规 `c08_d-10`；③ `ds_portrait_37` 整帧估角 dropout；④ 本轮 m1 在 5 张 c08 成片上失准。四条**同指该 session 的成片/眼区对多种方法都不友好**。
...
| 9 | `test/batch/p0_output_residual.py:309-319` | 可信度排除闸**无法区分「m1 错」与「引擎错且 m1 大致对」**：`c08_d-10`（真值 −18.01、引擎 `straightenDeg=0`、`rollSource=unavailable`、m1 读 −14.146 方向正确）被同一闸当成"测量不可信"排除。本轮靠 P0.3b 独立捕获，未漏判。修法：排除行与 `rollSource` 交叉判定，`unavailable` 行单列。见 §2.4C。 | qa-batch |
