...
///
/// 曾是 3°（避免小角度重采样）。P0 用户反馈（Windows 真实照片成片可见
/// 歪斜）后用合成小角度系列实测（`dev_roll_probe.dart` / `dev_roll_probe2.dart`，
/// 黄金集 g01/g03/g07 人工旋转 ±1–4° 跑全管线）：1–3° 的真实倾斜在旧死区下
/// **完全不修正**，是成片可见歪斜的最大单项来源；而干净脸的眼线估角在
/// 1–4° 区间误差 ≤1.1°，旋转到位后残差比死区放行小得多（g01 平均残差
/// 1.04° → 0.61°）。镜框脸的眼线估角在小角度下抖动可达 ±4°（ml-porting
/// 侧问题），但即便如此死区 1° 的平均残差也不劣于 3°（1.65° vs 1.49°
/// 的差距在估角噪声范围内，max 4.28° vs 4.10°），且系统性收益覆盖大多数
/// 无镜框照片。故收窄到 1°——旋转重采样用 mip 预滤波 + 双线性，1° 下的
/// 插值损失可忽略。
const double kRollDeadZoneDeg = 1.0;
