...
///
/// 用途：`removeBackground` / `detectFace` 在解码**之前**先拿尺寸——
/// 4958×7017 的扫描件如果直接全分辨率解码，RGBA + 中间缓冲会把进程峰值
/// 内存顶到 800MB+（G4.7 FAIL）。先读头、超限就按引擎工作分辨率降采样，
/// 全程不碰全尺寸像素缓冲。
///
/// `lib/ui/util/image_size.dart` 有一份同逻辑的实现，但它属于 ui-woodcraft
/// 领地不能 import，这里按同一套口径复制（连"返回摆正后尺寸"的语义一起），
/// 并有 bench 探针保证两个解码器（image 包 / dart:ui）与它一致：
/// 两个解码器都会在解码时烘焙 EXIF orientation，所以这里的宽高必须同样
/// **返回摆正后的尺寸**，否则降采样目标尺寸会和实际解码结果对不上。
///
/// 支持 JPEG / PNG / GIF / BMP / WebP(VP8, VP8L, VP8X)；其它格式返回 null，
/// 调用方退化到"解码后才知道尺寸"的旧路径。
library;

import 'dart:math' as math;
import 'dart:typed_data';

/// 摆正后的像素尺寸。解析失败返回 null。
