...
abstract final class MuZhaoFonts {
  ...
  /// 子集覆盖的**非 ASCII**字符，共 513 个；此外还包含全部 ASCII 可见字符
  /// U+0020–U+007E，合计 608 个。
  ///
  ...
  static const String coveredCharset =
      ...
      '；？￥　'
      // PHASE6 W1 追加（dart run tools/charset_scan.dart 产出）：关于页文案
      // （版/任/传/作/你/刘/承/林/永/源/络/网/诺/隐/页/题/首/清/通…）与
      // lib/core 控制台日志用字。缺失明细见该扫描器的输出。
      'Δ≤临产任传位作你刘剖功各哈复审将就工希引承控擎断林核永池汰沅淘清源滞热'
      '版物率盘私算络绪缓网落诊诺跨辨近通逼重锚键隐需页预题首'
      // 2026-09-19 追加（dart run tools/charset_scan.dart 产出）：抠图档位拨杆
      // 文案（快/速/精/档/较）与 ml-porting 双模型抠图在研代码的引擎侧字符串
      // （θ/φ/⚠/倾/闸/钳/膜/拟/掩/散/校/准/基/槛/模/眼/趋/轴/逆/顺/须…）——
      // 引擎 errorMessage 由 UI 直接渲染（CONTRACTS §6），必须入子集。
      // ⚠(U+26A0) 与 ↳ 一样在源字体里无字形，仅控制台日志使用，不作为缺字。
      'θφ⚠之也会估修倍倾具决准列则制叠只号哪基天夹微必快怕性感报拟推掩敏散施'
      '校样档槛模此然生眼管精而膜警证该走趋轴较逆速钳闸际随零顺须️';
}
