...
- 根因：**两边的夹具不是同一份字节。** gatekeeper 的探针在内存里旋转
  `C:\Users\liuyu\Pictures\a.jpg`（483×604，`p0_probe_raw.json` 的 `src_size` 与
  原图逐值相同）；qa-batch 落盘的 `out/P0_anchors/c05_d-10.png` 虽同为 483×604，
  但其未旋转版 `c05.png` 是 719×900（原图放大 ~1.49×），像素 MAE 19.29。
  亚像素级的插值/缩放差异落在这个样本的瞳孔 blob 门限上，判决直接翻面。
