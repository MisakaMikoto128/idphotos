...
## 2026-09-17（imaging）**可见的切边与"延展出来的假像素"是同一块像素 —— 而它由一条不等式决定，不由算法决定**

只读分析得出（**本轮未改代码**，全部数字可在 `out/P0_anchors/composed/` 上复算）：

**1. 切边不是旋转造成的。** `p1_upright` / `c05_upright` / `c08_upright` 的
`straightenDeg` 都是 **0.0**（roll 在 1.0° 死区内，**一度都没转**），切边照样存在：
底部最下一行内容的 y 值直方图有宽平台 —— p1_upright **y=518（91%）163/306 列**、
c05_upright **y=436（77%）168/309 列**、c08_upright **y=402（71%）300/381 列**，
线上方是主体（[235,235,235] 毛衣 / [220,173,147] / [34,20,19] 深色衣），线下方是纯底色。
旋转只改这条线的**斜率**（已在诊断轮验证到 ±0.0001），不改它的存在。

**2. 判据：ρ = 头高 / 源图高。** 证件照要求头顶留白 0.10、头高比 0.62 ⇒
所需裁框高 = `head_h / 0.62 = 1.613·head_h`；它能放进源图 ⟺ `1.613·head_h ≤ H` ⟺ **ρ ≤ 0.62**。
实测 **p1 ρ≈0.68、c05 ρ≈0.81、c08 ρ≈0.87** —— 三张全部越界，这是
"源图是一张紧构图头像"的直接后果，**任何裁剪算法都变不出不存在的像素**。
旋转再压一层：轴对齐裁框要装进**旋转后的源矩形**，容量
`h_max = min(W/(c·a+|s|), H/(|s|·a+c))`（p1 的 W/H/θ 下 = **1550.7** px，而未旋转是 2048）
⇒ p1 的 ρ 阈值从 0.62 降到 **0.47**。**摆正本身会吃掉 24% 可用高度**，这是旋转的隐性代价。

**3. 可见 ⟺ 主体压在那条边界上 ⟺ 该边界行的 alpha>0。** c08 家族 alpha 的实测：
**上/左/右三边 alpha 恒为 0，只有下边有主体**（533/1280 列，主体处均值 209，77% 的像素 >200）。
所以："往哪个方向延展会造假" 与 "往哪个方向看得见切边" 是**同一个方向**。
**切边可见的区域，正是延展必须凭空编造主体的区域；延展安全的区域，本来也没有切边要修。**
这是定理不是巧合 —— 见 `render.dart` 的越界填色分支与 alpha=0 判据。

**4. 在国标带内能挪动的量是有上限的，而且很小。** 两个比例是两种独立"货币"：
头高比 0.60–0.65 给了 `1−0.62/0.65 = **4.6%**`（≈26px @567）；头顶留白 0.07–0.12 给了
`+2%`（≈11px）。合计 **≈37px**。而实测底部缺口：p1 **49px**、c05 **131px**、c08 **165px**。
**p1 能靠"重新构图"填掉约 3/4，c05/c08 只能填掉约 1/5 —— 剩下的只能编，或者放宽标准。**
（现状：`p1_upright@visa_us` 的 `achievedHeadHeightRatio = 1.113` 说明二级收缩路径
**根本没有band 检查**，它已经在输出远超标准的构图。）

**5. 度量 `outOfBoundsFraction` 的 θ 依赖会让排序反向。** 它按 **AABB** 算，
而 AABB 恒 ⊇ 源矩形 ⇒ **metric ≤ 真值，θ=0 时取等**。实例：同一族
`c08_d-5`（θ=−12.79°）报 **13.5%**、`c08_d+5`（θ=−2.85°）报 **25.9%**，
而两张的真实底部缺口**都是 ≈29%**（upright 的 29.1% 与平台位置完全对上：165/567）。
**越摆正，报出来的越界越小 —— 判据与被判对象反相。**

---

## 2026-09-17（qa-batch，r3 重测）**"谁改写了判据输入"我记错了人 —— 归因推断也能长成事实的样子**

`out/GATE_P0_r3.md` 把 r3 作废归因于"gate 之外的一个进程在判定中途重写了 compose 产物（推断为
ml-porting，它刚改完 `iris_roll.dart` 要自查）"。**推断方向错，人错了**：写那两份产物的是
**我（qa-batch，裁判）**，在跑主会话授权的 r3 重测。证据是三条 mtime 对齐：
我的 `out/r3_logs/02_compose.log` 尾部 `All tests passed!`、`P0_compose_items.jsonl` 08:56:22、
`P0_compose_summary.json` 08:56:23，整段落在报告自述的 08:51:26–08:57:30 窗口内。

**这条值得记的不是"谁干的"，是形态：**

1. **推断被写进了永久日志，而它长得和结论一模一样。** 原文自己标了"这是推断（mtime 只能证明文件
   何时被写，证明不了是谁写的），但方向明确" —— 该说的都说了，**下一个读的人只会读到
   "实现方在验收窗口重跑脚本"**。凡是要落进 `PITFALLS`/判决的归因，**没有身份证据就不要写身份**，
   写"某一进程在窗口内重写了 X（作者未知）"。
2. **真正的结构问题被这个错归因盖住了。** 不是"被判的一方篡改证据"，是**裁判与门禁同时读写
   同一份共享产出**：`out/` 没有占用约定，两个 agent 谁都不知道对方在跑。这类撞车会**反复**发生，
   而按"人的问题"处置就只会得到一句提醒，下次照撞。**给 `out/` 的产物加写入者标记，或规定
   重跑前先登记** —— 这是可执行的；"注意时机"不是。
3. **"我不知道是谁写的"与"我知道不是谁写的"要先分开。** 我这次能给定论，只因为我是那个进程、
   手里有自己的日志。若无日志，正确的写法是"我来过这个窗口"而非"是 X 写的"。

---

## 2026-09-17（qa-batch）**同一个 64 位指纹，Dart 打有符号十六进制、Python 打无符号 —— 并排显示＝假漂移**

`out/GATE_P0_r3.md` 里 `:55/:168` 写当前树摘要 **`-32be85b2dab13e3e`**，`:558` 的逐文件核对却把
**`cd417a4d254ec1c2`** 判为"0 处不符"。两个值看着毫不相干，**其实是同一个 FNV-1a 64 值**：

```
0x10000000000000000 - 0xcd417a4d254ec1c2 = 0x32be85b2dab13e3e
```

`tools/gate/provenance.dart:385` 用 `h.toRadixString(16)`，Dart 的 `int` 是**有符号** 64 位，
`& 0xFFFFFFFFFFFFFFFF` 在 Dart 里等于 `& -1`（不减位），高位置 1 时 `toRadixString` 就带出 `-` 号；
Python 侧 `format(h, "016x")` 打无符号。**同一个数，两种字面量。**

**可操作：**
- 结论：**这不是漂移，别去追**。判据走 `blobHashes` 逐条比是对的（`provenance.dart:376` 明确
  "不与生产者的 `fnv1a64` 做跨实现比对"）—— 那条路给出了正确答案。
- 但**人读的那两行会把人带偏**：同一份报告里两个摘要值并列，任何读者都会读成"代码变了"。
  要么统一成无符号，要么在打印处标注"有符号渲染"。
- 一般化：**跨实现比较任何摘要之前，先确认两边的渲染口径（有符号/无符号、大小写、补零、
  字节序、是否含路径）**。数值相等而字面量不同，是本项目"看起来验证过"的又一个入口。

## 2026-09-17（imaging，勘误）**"平台线"是几何事实，"它是什么"是另一回事 —— 我用前者签发了后者的结论**

我把 `c11_d+10` 报成"头发被切平"，主会话看图反驳，**我复算后确认它对、我错**。
我的扫描器量到的是真的：**y=0 处 69 列共享同一条上边界**。
但那条边界是**一块与主体不相连的碎片贴着画幅上沿**，不是发际线 —— 而我**从没看过那块区域是什么**就写了结论。
（目视复核也错一次：我只裁了成片**顶部 200 行**，脸落在窗口下沿，我就推成"上方没有头发"。**窗口取错 ⇒ 结论反了。**）

**复算（可复现）：** 成片 `c11_d+10` 有 **3 个连通块**（主块 121568；1779 @x143..211 y0..33；404 @x72..94 y45..80），
**不旋转的 `c11` 只有 1 块**。碎片放大看是**肤色+深色的一个头的局部**。

**同一批里第二个例证 `c01_upright` 也归错了类。** 平台线在 y=45，我判"裁框切了头发"；
去看**引擎实际读入的画布** `out/P0_anchors/c01_upright.png`：**第 0 行中带就有头发像素**（[91,81,75]、[92,86,90]），
原始源图 `20240710193717_7c99.jpg` 同样第 0 行即有内容 ⇒ **源图自己把头顶切了**，规格要的 7–12% 留白在原图里不存在。
**"源图切 vs 流水线切"不用推理，去读输入画布的第 0 行即可。**

**可操作：**

1. **长直边界只证明"这里有一条合成边"，不证明"被切的是什么"。** 报告里必须同时给**边界两侧的内容**
   （颜色/在主体上的位置/是否与主体连通），否则这条测量会被当成它支持不了的结论。
2. **"非主连通块"不等于"另一个人"。** 全库 100 个输出里 ≥200px 的非主块命中 **22 个输出**，
   逐个放大看只有 **6 个**是别人（`c12`×5、`c11_d+10`×1），**13 个是主体被抠图洞切开**（c08 家族 **8/8**），3 个是边缘残片。
   **判据要落在"那块前景是不是人"，不是"有没有第二块"。**
3. **这个类不随旋转才出现。** `c12` 与 `c12_upright`（**`straightenDeg=0.000`，没转**）一样带着邻人的脸
   ⇒ 根因是**源图是合影 + 取景把邻人框进来**，旋转只改变取景。**别把"多出别人"记到旋转账上。**
4. **想核对源图时，夹具 id → 路径是现成的**：`out/P0_input_hashes.json` 的 `byId` 是输入字节 sha256，
   与 `test/dataset.json` 的 `items[].sha256` 直接对（本次 13 个 base 夹具 **9/9 命中**，
   顺带纠正：`c05` = `C:\Users\liuyu\Pictures\a.jpg`，不是 `2.jpg`；`p2` 才是 `2.jpg`）。
5. **图省事的窗口会骗人。** 要看"某条边界上方/下方是什么"就裁到边界两侧，别拿整图的某个固定窗口当证据。

---

## 2026-09-17（gatekeeper，r3）**一条正确的豁免理由过期之后，会变成一条错误的豁免**

主会话 2026-09-17 定过一条贴线规则（`tools/gate/gate_P0.dart:275`、`:892-900`）：

> 余量 < 量具误差的条目，**单支量具的 FAIL 不足以定罪** —— 须第二支独立量具复现同一超线才计违规；
> 只有一支超线则记「边界噪声 / 证据不足」，**不消耗实现方修复轮次**。
> **本规则不适用于 P0.3a**：它**超阈值 7 倍以上**且**两支量具独立复现**，是确定性违规。

**那段豁免理由描述的是 r1 的事实**（P0.3a 当时 11.368°，约 7.5 倍；两支量具都读到）。
r3 时 P0.3a 是 **1.7534°，1.17 倍**，且**第二支量具（SIFT 1.2610°）在阈值之下** ——
**豁免所依据的两条，本轮一条也不成立。**
于是这条规则**按它自己的文字本该覆盖本轮 P0.3a，却因为一句没被更新的排除语而不覆盖。**

**可操作：**

1. **豁免/例外条款必须带"失效条件"，不能只写"因为当时是 X"。**
   "不适用于 P0.3a，因为它超阈值 7 倍以上" 是**把一次观测写成了规则**。
   **正确的写法是写条件**："当 margin > instrumentErr 且两支量具均超线时，按确定性违规处理；
   否则适用贴线规则" —— 这样它自己会随读数变化。
2. **绝对量的例外会随相对量失效。** "7 倍以上" 是相对阈值 1.5° 说的；阈值没动，**读数动了**，
   豁免就悬空了。凡写"因为超标很多/很少/从未发生"，都要问**这个前提会不会变**。
3. **判据冻结 ≠ 判据永远正确。** 阈值只有主会话能改是对的（防作弊），
   但代价是**判据里的错误陈述也会一起被冻住**，且**每一轮都会被执行一遍**。
   发现这种冲突时：**按冻结判据出裁决，同时把冲突原样上交**，不自行适用、也不沉默。
4. **这一条与"门禁不下场"是同一条纪律的两面**：我不能改判据，**但我必须让判据的错处可见**——
   **沉默地执行一条自相矛盾的规则，等于替它背书。**

## 2026-09-17 imaging：同一份缺失量在两个像素网格上（画布 px / 成片 px）

**坑**：讨论「把 `cn_big_1inch` 从 0.10/0.62 换到带内最优 (0.07,0.65) 能补上 p1 底边缺口的多少」时，
一边给出「收益 ≈ 0.0208 · head_h ≈ 29px」，另一边给出「缺口 49px」，两个数**是在不同网格上量的**：
29px 是**摆正前画布**上的像素，49px（实测 48）是**成片自身**（390×567）的像素，
两者相差 `crop_h_canvas / 567` ≈ 3.96 倍。直接用 29/49 ≈ 58% 会得出「带内能补掉一半以上」的错印象，
实际是 ~15%（画布口径 29/194）。

**为什么容易踩**：两个数都叫「px」，都讲同一张图、同一件事，**没有任何一个字段名会把网格标出来**。
`achievedHeadHeightRatio` 是唯一自带换算的锚：成片里 `head_h = q · spec.heightPx`（0.62×567 = 351.5），
由它一步就能把任意以 head_h 为倍数的量换成成片 px，不必重建裁剪框。

**又一坑（同一处，更深）**：换到成片 px 后收益**还要再缩一次**。裁剪框变短 ⇒ 同一 390×567 下
放大率上升 `q_new/q_old` ⇒ **留下的那条缺失带也被一起放大**。所以

    d_out' = ( d_src − Δk · head_h_src ) · 567 / crop_h_new

不是 `Δk · head_h_out`。p1_upright：48 → **42.6** 输出px（−5.4px，−11%），而不是按 head_h 直乘得的 −7.3px。
**量「补上了多少」时，被补的对象自己也会随缩放变化 —— 先写出含缩放的闭式，再代数。**

**实测（可直接复核，用来钉住「缺口 48」这个数）**：`p1_upright__cn_big_1inch.jpg` 无旋转，
底边往上连续纯白带在 x=160..280 恒 48 行、边界是一条**水平线** y=518 ⇒ θ=0 时缺口是常数，
正是「溢出深度」的定义值。`p1__cn_big_1inch.jpg`（θ=−4.4631°）同一条边界变成**斜率 +0.075 的斜线**
（x=0→505，x=280→526；|tan 4.4631°| = 0.0781，差 4%）⇒ **同一份缺失，水平线 vs 斜线：旋转改的是形状，不是量。**

## 2026-09-17 imaging：多脸类——主脸选择早已实现，别重设计；真正的缺口在合同

**坑**：被问到「多脸时主脸怎么选（面积最大？最正？最居中？）」时，第一反应是设计一套选择规则。
实际 `lib/core/matting/yunet_decoder.dart:144-157` 的 `pickSubjectFace` 已经定型：
`score = area × (1 − 0.5·d)`，`d` = 脸心到画面中心距离 / 半对角线，注释里写明已在冻结集上核过
（三张五人合影里自拍者恒当选）。**从零设计一套规则，只会推翻一个已经有人核过的选择，且拿不出样本。**

**真正缺的东西**：`lib/core/api.dart:154` 的 `FaceInfo` 是**单数**，`detectFace` 返回 `FaceInfo?`，
而 `matting_worker` 内部 `nonMaxSuppression` + `pickSubjectFace` 手里**本来就有整张列表**，
是 `toFaceInfo` 把它丢掉、只回一个 ⇒ **信息是在最后一跳被丢的，不是在检测侧不存在**。
于是 `compose` 无法对第二张脸施加任何约束，`c11`/`c12` 的「画幅里出现邻人」只能眼睁睁发生。

**教训**：判断「某个能力有没有」之前，先往**上游一跳**看它是不是被丢在返回值里了。
列表 → 单值的收窄点，比「模型不支持」常见得多。

**另一条（口径）**：「画幅里出现了非主体人物」这个类，**不是旋转特有的**——
`c12_upright`（`straightenDeg = 0.0000`、`oob = 0.0000`）成片左侧就站着邻人半张脸（含眼镜），
`c12_d+3`（`straightenDeg = 0.0000`）同样有 3860px 的邻人块。**看的是成片，不是统计推断。**
且按「与主体不相连的第二块前景 ≥200px」全库数得 23 个输出、其中 10 行属本类，
但这 10 行**只来自一个多人物来源**（同一张五人合影的三个导出）⇒ **发生率未测，不许写「罕见」。**

## 2026-09-17 imaging：往 PITFALLS 追加中文条目，别用 `python -c \"...\"`

两次栽在同一处。`python -c "…"` 里那段脚本是被 **shell 双引号**包着的，
脚本正文中的**反引号会被 shell 当命令替换先执行掉**（`` `cn_big_1inch` `` → `cn_big_1inch: command not found`），
于是写进文件的正文**缺字、且不报错**（本次静默吞掉十余处，行数照增，`startswith(before)` 仍为真）。

**做法**：正文先写进临时文件（Write 工具，不经 shell），再用一小段 `python -c` 只做「读 A、读 B、写 A+B、校验」，
脚本里不出现正文。**校验不能只看行数或前缀** —— 纯追加天然满足这两条，
要**回读正文并与源串比对**，或至少确保正文从未经过 shell。

## 2026-09-17 imaging：裁剪框装不下源图时，卡住它的是**宽度**，不是留白

**坑**：讨论「带内最优 (0.07,0.65) 能不能消掉 p1 的底边切边」时，双方都在**留白 r** 这一个旋钮上推：
「留白得到 0.186，比上沿 0.12 高 0.066 ⇒ 完全消掉切边走不到」。这句话**只对了半条腿**。

裁剪框的两条边是分开约束的：

    底边：crop_bottom = headTopY + (1−r)/q · head_h      ← 受 r 控制
    宽度：crop_w      = a · head_h / q                    ← **与 r 完全无关**

`crop_left/right` 由「人脸水平居中」定死，所以宽度只由 q 决定。于是：

- 消掉**底边**切边：`(1−r)/q ≤ (H_canvas − headTopY)/head_h`；p1 上 = 1.31485，q=0.62 时 r ≥ 0.1848。
- 消掉**左右**溢出：`a·head_h/q ≤ W_canvas` ⟺ `q ≥ a·head_h/W_canvas`；p1 上需 **q ≥ 0.766**。

**后者才是 binding constraint**，且它是 q 的下界、与 r 无关：把 r 从 0.10 加到 0.5 也一动不动。
p1 的数是 q ≥ 0.766（头占画幅 77%），而带内上限 0.65 ⇒ **差 0.116，这个旋钮根本够不着。**

**为什么会这样（值得记住的形态）**：源图**本身已经是证件照构图** —— 用两组已记录/已测量量标定出的
`head_h = 1319`、`headTopY = 313`（画布 1184×2048）⇒ **头已占源图高 64.4%，比目标 62% 还大**。
再叠加 `a_crop = 0.6875 > a_source = 0.578`（裁剪比源图更宽），任何达标框在**两维上都比源图大**：
实测 **1463×2128 vs 1184×2048（×1.235 × ×1.039）**。⇒ **pan 无解、缩也缩不进。**

**教训**：判「这个取舍能不能达到目标」之前，**先把约束按维度拆开写，看哪一维先卡住**。
两个约束都写成"花预算"的样子时，很容易以为它们是同一个旋钮 ——
**实际一个受 r 控制、另一个完全不受 r 影响。** 只在一个旋钮上推，会得出"再推一点就到了"的错结论。

**方法（这次比上次可信）**：不再用地标重建，改用**两个互相独立的已存在量**解几何 ——
成片上**实测**的底边填充带（48 输出px）+ 盘上**记录**的 `outOfBoundsFraction`（0.2591），
两个未知数两个方程。**并主动报残差**：同参数预测 `@cn_1inch` 得 39.1px / 0.2947，实测 35px / 记录 0.2866
（差 11% / 3%）⇒ 模型够判方向与量级，不足以当精确读数。**解出来能对上两个独立来源，比"重建得像"强。**

---

## 「窗口内无人写入」这句话，如果没有机器核过，就只是一句自述 —— 门禁读的文件不在快照集里

**谁**：gatekeeper　**时间**：2026-09-17　**代价**：一整轮 r3 作废 + 一次错归属上报

**事实**：r3 的两轮测量里，判据输入 `out/P0_truth.json` 都在我的窗口内被改写过
（09:12:41 与 09:22:10，写入方是 `test/batch/p0_finalize_v1.py`）。
**而我的窗口稳定性快照核不出这件事** —— `out/hashes_P0_r3_pre.txt` / `out/hashes_P0_r3.txt`
各 43 条，逐条比对**零差异**，因为快照集里只有 `out/P0_truth_v0.json`（手写件）
与 `out/P0_truth_leaves_baseline.txt`，**没有被判据实际读取的生成件 `out/P0_truth.json`**
（它在 2026-09-17 换靶「内容钉 → 钉手写分母」时被移出了快照集）。

**⇒ 门禁读一个自己不核窗口稳定性的文件。快照比对"零差异"因此不构成任何保护。**

**教训**：**读一个文件，就该在窗口两端各哈希它一次。**
「本轮窗口内无人写入 X」是一句需要机器支撑的断言，不是一句可以顺手写下的旁白 ——
它与 [[#「不存在/没有/为 0」的结论先确认那个集合里本来该有它]] 同形：**缺席证据与缺席论证字面一样，区别只在分母。**
快照集的分母必须**按「判据实际读了哪些文件」生成**，不能按「哪些文件看起来重要」人工圈定。
**一条快照规则一旦靠人工维护，它迟早会与真实读取集分叉，而且分叉时它报的是「零差异」。**

**副产物（更该记的）**：我用"gate 只 spawn `git`"这条**错误依据**去排除本门，进而把改写
**归属给了 ml-porting**，并在报告里写成「被判方在判定中改写判据输入，结构上让证据由被证者控制」。
两处都错，已撤回。**依据错在范围**：我只 grep 了 `Process.runSync`，
而 gate 通过 `runProcess(...)` spawn 了四个外部进程（两个 Python 量具、一个 overlap、一个 `flutter test`）。
**一次 grep 漏了一种调用方式，就得出一句关于进程归属的结论** ——
与 [[#范围要由命题的主语给]] 是同一个病的第四种显形。

**正确动作**：查「谁写了这个文件」时，**先把「本门 spawn 了哪些进程」枚举完整**
（按**调用方式**枚举，不是按你记得的那一种），再谈排除。

## 2026-09-17 imaging：共享工作树里，「我只改自己的文件」保护不了任何人的测量轮

**事故**：imaging 在自己范围内 `git apply` 恢复了被主会话回收的 WIP（改了
`lib/core/imaging/compose_engine.dart`、加了两个 dev 探针），**同一时刻主会话正在跑
P0 全库合成测量**。那一轮结束时 provenance 判
`codeStableDuringRun:false` / `roundValid:false`，
`changedFiles:["lib/core/imaging/compose_engine.dart","lib/core/imaging/dev_relax_corpus.dart",
"lib/core/imaging/dev_relax_probe.dart"]` —— **逐条点名了 imaging 写的文件**，
整轮数字作废、必须重跑。量具判得对，不是误报。

**为什么两边都没有察觉**：
- 写的一方（imaging）看 `git status` 只看到自己的改动，**看不出有人在读**；
- 跑的一方（主会话）看进程表只看到自己，**看不出有人在写**；
- 被测集是 `lib/**`（`test/batch/code_fingerprint.dart:31-32` 的 `kMeasuredRoots`），
  **任何人对 lib/ 的任何写入都污染别人的轮次**，不管改的是不是"自己的文件"。
  ⇒ CLAUDE.md §7.5 只覆盖了「两个 agent 同时改**同一个**文件」，
  **没覆盖「一个 agent 写、另一个在读」** —— 后者不会覆盖对方的代码，只会静默废掉对方的结果。

**为什么破坏性更大**：写的一方几秒就能重放，读的一方要重跑整轮（百秒级 + 排队 + 可能顶掉别的窗口）。
**成本严重不对称，所以默认应由写方让路**。

**正确动作**：共享工作树里要跑"读代码/读数据"的测量时，**启动前落一个显式锁**
（如 `out/<owner>/RUNNING.lock`，写起始时间与预计时长），任何人在动手写 `lib/` 前先查锁。
**光靠"我先看一眼 git status"是不够的** —— 它给的是「现在」，而冲突发生在「刚才」。
**并且：`git status` 干净 + 自己的文件不见了，有两种成因（被回收 / 自己没写成功），
字面上完全一样** —— 判不出来就去翻对方的暂存目录（本案是 `out/main_session/REVERTED_*`），
别急着重写。回收别人的 WIP 时，**除了存补丁，还要在对方看得见的地方留一条指名记录**。

## 2026-09-17 imaging：指纹域含 `dev_*` 探针 —— 改一个测试脚本会作废一个它影响不到的测量轮

**事故**：按派活适配 `dev_selfcheck.dart` 的夹具角（只改测试脚本，生产代码一行未动），
跑完自检得指纹 `5dc268f851ddf4ea`，而成片台停在 `c675ca00967f7202`
⇒ `sameCodeStateAsCompose:false`，按 r2 规则这两半不得拼进同一份判决，得重测一半。
**那一轮的测量对象是 `lib/**` 的生产代码，我一个字节都没碰**，却被判为不同代码状态。

**根因**：`test/batch/code_fingerprint.dart:31-32` 的 `kMeasuredRoots = ['lib','test/batch']`
配 `kMeasuredExts = ['.dart','.py']` ⇒ **`lib/` 下全部 `.dart` 都在域内，包括 `dev_*.dart` 这类
只被 `flutter test`/`dart run` 直接执行、没有任何生产文件 `import` 的探针**
（核实：全仓无一处 `import` `dev_selfcheck.dart`，其余提到它的都是注释 ——
`compose_only_engine.dart:7`、`dev_synth.dart:62`、`fonts.dart:83`）。
指纹回答的是「被测源码有没有变过一个字节」，**不是**「这个改动有没有可能影响测量」；
二者在「改的是测试脚本」时正好分叉，而指纹这一侧只会说"变了"。
⇒ 我 14:42 那次作废的 `changedFiles` 里出现 `dev_relax_corpus.dart`/`dev_relax_probe.dart`，
**同因**：那两个探针当时也不可能影响成片残余的读数。

**正确动作**：
- 要改 `lib/**/dev_*.dart` 前，**先当成"这一改会作废在跑/刚跑的测量轮"来报**，别等 provenance 判出来。
- 判"行为没变"要用**独立证据**（grep 全仓有没有 `import` 它）。可以说「指纹动了、行为不可能动」，
  **但不能说两者等价** —— 等价与否由轮次主人按规则判，不由改文件的人判。
- **不要为了消除不一致去重跑成片台**：那会重写被钉产出，把一轮有 provenance 的测量
  降级成一次没有 provenance 的覆盖。重测哪一半是主会话/门禁的决定。

## 2026-09-17 imaging：夹具角钉在阈值上 —— 而且"钉在哪一侧"没有被断言

**事故**：10° 摆正死区落地后 `dev_selfcheck` 从 9/9 变 7/2，两条失败全是夹具角
`[-10,-6,6,10]` / `[10,-10]` **与死区门槛恰好重合**（`planRotation` 用
`roll.abs() <= deadZoneDeg`，**取等号即不转**）⇒ 夹具落进死区，量的是恒等变换，断言必炸。
门禁 2B.8 的 ±10° 是同一缺陷的另一半，已先挪到 ±15°。

**为什么"写死数字"是病根，"改数字"不是解药**：改数字只修好这一次。
下次 `kRollDeadZoneDeg` 再动，夹具还是静默失效 —— 而且失效方向是最坏的那种：
**不报错，只是从此量一个恒等变换然后"通过"**。

**正确动作**：夹具角引常量（`kRollDeadZoneDeg * 0.6`），**并显式断言夹具确实落在期望的那一侧**
（`expect(roll.abs() > kRollDeadZoneDeg)`）。断言把"夹具位置"从一条无人复核的约定
变成一条会响的判据。凡夹具值必须与某个产品常量保持大小关系时，**两边都要钉**：值引常量，关系断言。

**同族（本项目当天第七次以上）**：这次 `dev_gate_repro.dart` 的「门禁同款」是同一形态的又一例 ——
**名字声称的口径宽于它实际钉住的范围，而它读起来完全正常**。它写着"门禁同款"，
钉的却是 `[10,-10,6,-6]` 四个全落在死区内的角，量的是恒等变换。
比前几次更狠的一点：**它没有 `expect`，所以它连失败都不会失败** ——
不会变红、不会超时、不会报错，只会安静打印一组看起来正常的残差，
等着下一个人拿它当门禁读数用。**沉默比报错更容易被当成"没问题"。**

**⇒ 凡"复现 / 镜像 / 同款"这类名字，必须去核它镜像的那个源现在是什么值**
（本案的源是 `integration_test/compose_eval_test.dart:270`），
并把"两处必须同步改"写进文件头；可能的话把关系做成断言。
**名字是承诺，不是描述；承诺没人对表，就会静默过期。**

## 2026-09-17 gatekeeper：边界带取"所有 9°–11° 的样本"还是"与 needs-straightening 集合的交集"—— 两个数都错，而它读起来完全正常

**事故（本门禁自己犯的，被自检前的复核逮到）**：P0.3a② / P0.3b 的样本集是
"真值 |tilt| > 10° 的夹具"（15 条），边界带是"真值 9°–11° 只报数"。
我最初把边界带写成 `rotated.where(inBoundaryBand)` —— **全部** 9°–11° 的夹具，9 条。
于是报告里那句话会变成：

> 需要摆正 15 条（含带内 **9** 条）⇒ 实际参与判定 **6** 条

而 ACCEPTANCE 定的口径是 **15 / 11**（15 条去重夹具 − 4 条带内 = 11 条参与判定）。
**两个数都错**，但句子结构、单位、语气全都正常，肉眼读过去没有任何异常。
根因是**取"集合的谓词"而不是"交集"**：`inBoundaryBand` 描述的是样本的一个属性，
判据要的是**这个属性在当前样本集内**的那部分。

**为什么带内侧（9°–10°）的 5 条不是"带内豁免"**：它们压根不在
"必须摆平"的判据里 —— 归 P0.6 管。把它们算进 P0.3b 的豁免数，
等于**让一条判据去豁免一批它本来就不判的样本**。

**同一条规则在 P0.6 上要反着理解**：P0.6 的样本集是**死区内**，
所以它的边界带**正是带内侧那 5 条**（`p1_d-5` / `c07_d+10` / `c02_d+10` / `p2_d+10` / `c11_d+10`）。
`c11_d+10`（真值 9.89°、自报 10.283°、**转了**、成片残余 −0.39°）就是这条豁免存在的唯一理由：
按次判据它 `|残余 − 真值| = 10.28°` 会被判 FAIL，而它其实只在边界上。
**⇒ "边界带"不是样本的属性，是"样本集 × 属性"的关系**；
跨条目复用同一个谓词之前，先问这个条目的样本集是哪一侧。

**配套**：报告里的 15 / 11 这类**算术**，要用**现算**的三个计数拼出来
（全规格行数 / 门禁规格去重条数 / 带内条数），不要写死。
写死的话，语料一变它就会变成一句读起来很正常的假话 —— 与
「豁免条款的理由是当时的状态描述」同族（见 `tools/gate/gate_P0.dart` 的贴线规则）。

## 2026-09-17 gatekeeper：新判据必须自证"真的会分类"，否则它的绿和"没跑"长得一样

**背景**：P0.6（死区内不得被转动）是随 10° 死区新加的条目。它此前**从未在任何输入上跑过**。
一条从未执行过的判据脚本，`PASS` 与"根本没跑"在人读的报告里完全同形。

**做法**：`tools/gate/p0_p06_selftest.dart` 用**构造输入**（不经生产代码、不读盘上产物）
把六种结局各跑一遍并要求各自的分类：自洽 PASS / 死区内被转动 FAIL / 量具读错但已独立复核
（挂牌 + 扣分母，仍 PASS）/ **无第二支量具（必须 MANUAL，不许静默通过、也不许静默判失败）**/
死区外夹具不得混进来 / 边界带只报数。
运行入口是 `dart run`（**不是** `flutter test`，见前条 `main(List<String>)` 那笔）。

**为什么这次的"量测失效"分类不是自欺**：判据能机械地分开"量具坏"与"引擎真的乱转了"，
只因为**裁剪+缩放是相似变换、不改变角度** —— 引擎自报没动它时，成片倾角必然 ≈ 真值，
两支独立量具仍不一致就只能解释为量具故障。**这条推理有前提**；
一旦流水线里出现任何非相似变换（例如更激进的形变类裁剪），它就会失效，
届时"量测失效"这个归类会变成藏缺陷的口袋。**判据的每一步都要能说出它靠的是什么不变量。**

## 2026-09-17 qa-batch：Radon 搜索的**轨道端点**会被当成读数写进产物

**现象**：`p0_lib._radon_orientation()` 在 `angs = np.arange(-span, span+1e-9, step)`、
`span = 16.0` 上找脊线。峰值落在轨道端点时函数**照常返回 `deg = ±16.000`**，
与一个真实的 16° 读数**在产物里长得完全一样**（同样是一个 float、同样带 `peak_ratio`）。
r4 实测 4 行命中：`c08_d+5` −16.000、`c08_d-5` 15.904、`c08_d-10` 16.000、`c08_upright` 16.000。

**为什么危险**：这些值被下游写进 `tiltAbsMaxDeg`，而该字段的注释是
「保守上界：所有可用方法里 |tilt| 的最大值。门禁要保守时用它」。
拿它当保守上界 ⇒ 把一个**搜索边界**当成真实的大角，方向恰好是"更严格"，
所以它不会引起争论，只会静默地把样本推向失败侧。判据若据此改判，改的是噪声。

**正确动作**：`measure_m2` 收到 `|deg| == span`（或 `|deg| > span - step`）时应报
`rail_hit` 而不是返回一个角度；调用方按"测量失败"处理。**搜索有边界，
边界上的返回值必须能与真值区分**——否则函数在说"我没找到"，而调用方读到的是一个数。

**同族的另一条**：`p0_output_residual` 的两张违规表（死区内被转 / 死区外没摆平）
**不带 `inBoundaryBand` 标记**，而 r4 里命中这两张表的 2 条**恰好都落在 9–11 边界带内**
（ACCEPTANCE 明令只报数、不判 FAIL）。表的**名字**（`...Violations`）声称的口径
宽于它实际钉住的范围 —— 见队列项与 `out/QA_r4.md` §7。

## 2026-09-17 gatekeeper：空判定集有两个反面，站错哪一边都是整轮作废

**现象**：r4 跑到写报告时崩在门禁**自己的**断言上（`gate_common.dart` 的
"MANUAL 项不许 pass=true"），**一条判决都没产出**。根因 `gate_P0.dart` 的 P0.1b：
`pass = instrumentsOk && bad.isEmpty`、`manual = !instrumentsOk || judged.isEmpty`。
新 10° 死区下**锚点栏判定集结构性为空**（12 条锚点真值全在死区内，最高 `c08` 的 −8.01°），
于是 `bad.isEmpty` **空真** ⇒ pass 与 manual 同时为真。

**同一个空集还有另一个反面**：P0.5c 两个分支都是硬编码 `pass:false, manual:true`
（它读旧的 `out/gate_G2B.json` / `gate_G4.json` 判"有没有退化"，而读旧结果是拼判决，
所以它干脆不判）。而 `_finish` 的 `allPass = every(pass == true)` ⇒
**只要存在一条 MANUAL，整轮就不能 PASS**。两处合起来：**门禁永远不可能 exit 0**。

**教训（一）**：空集上问"有没有违规"恒答"没有"。`bad.isEmpty`、`every(...)`、
`where(...)` 的结果在空集上**全部恒真** —— 一个没有任何样本的项会安静地报 PASS，
而它与"真的检查过且干净"在人读的报告里**完全同形**。
补守卫时要注意方向：`judged.isNotEmpty` 进 `pass` 只是**半个**修法 ——

**教训（二）**：**"已记录的覆盖收缩"与"实现方永远不达标"必须分开**。
把 P0.1b 一律记 MANUAL 是"诚实"，但在 `allPass` 的语义下等于**永久停机**；
ACCEPTANCE 已明写"死区外那半边只由夹具承担、真实照片不再参与"，
所以正确写法是三态：锚点栏非空照旧判 / 锚点栏空而**夹具栏非空**则 PASS
并**写明覆盖由谁承担** / 两栏都空才 MANUAL。
**一条恒 MANUAL 的项不能充当 PASS 的门**，反过来一条"空集就算过"的项也不能。

**教训（三）**：修的时候**扫错了属性**。第一遍扫的是"pass 与 manual 能否同时为真"
（防**崩**），该扫的是"**判定集为空时本项声称什么**"（防**假通过**）。
按后者重扫才找出 P0.4（`manual` 只看 `instrumentsOk`，样本为空 ⇒ 两条违规统计空真
⇒ 静默 PASS 且**不挂牌**）—— 这一类**不会崩、不会报警**，比会崩的那种更难发现。

**教训（四）**：新判据仍要自证会分类。本次三条修改都进了
`tools/gate/p0_p06_selftest.dart` 的构造用例（7–10：能到 pass=true / 两栏都空 ⇒ MANUAL /
P0.3b 空集 ⇒ MANUAL / P0.4 空集 ⇒ MANUAL），**用 `dart run` 直调 `evaluateP0`、
不写 `out/`** —— 整轮跑会写 `out/GATE_P0_r*.md`，拿它当自证等于拿归档件做实验。

## 2026-09-17 gatekeeper：作废轮（VOID）对"有效轮才走到的分支"是零覆盖

**现象**：同一个崩溃点，两次干跑都没触发。因为 `pass = instrumentsOk && ...`，
而两次干跑都是**量具陈旧 ⇒ `instrumentsOk=false` ⇒ `pass=false`** ⇒ 断言不触发。
**只有一轮真正有效的运行**（量具新鲜 + 冻结输入 + 判定集为空）才走得到那个分支。

**教训**：拿一个 **VOID 的干跑**当"报告路径已自证"是无效的 ——
作废路径会**短路掉**后面所有分支，于是干跑越"成功"，它对有效轮的覆盖越接近零。
**干跑的绿色只证明作废逻辑本身在跑。** 要证明有效轮的分支，
要么构造输入直调纯判定函数（见上一条的用例 7–10），要么承认那段没被自证。

**推论（判据实现）**：把"没有读数"和"通过"都压进 `pass` 这一个布尔，
就会让两种完全不同的状态在所有下游读取处同形 —— 断言、`allPass`、报告读者都一样。
凡是要区分多态的地方，**布尔不够用就别用布尔**。

## 2026-09-17 gatekeeper：字段名猜错时，"对的答案来自错的理由"

**现象**：为核实"锚点栏有没有死区外样本"，我按
`s.get("truth", s.get("tiltDeg", s.get("truthDeg")))` 去读 `out/P0_truth.json`。
三个名字**都不存在** ⇒ 13 条锚点全部读成 `None` ⇒ 脚本打印
「`|真值| > 10` 的锚点数 = **0**」。

**危险在哪**：**这个 0 是正确的答案**（锚点真值最大确实只有 `c08` 的 8.01°），
但它来自一个完全错误的理由 —— 字段名没对上。而它**读起来完全正常**：
一个数字、一个符合预期的结论、一个刚好支持我论点的方向。
我若就此收工，等于拿"字段名猜错了"充当"已证实"。

**正确字段是 `trueRollDeg`**（同族还有 `expectedEngineRollDeg` / `truth_apply_deg`）。
按真字段重算才得到同一个 0 —— 这次它来自理由。

**动作**：
- 探一个**不熟悉**的产物时，**先 dump 一条完整记录**看字段名，再写任何统计。
  `.get(k, default)` 的默认值会把"字段不存在"**静默**转成"值缺失/为 0"，
  与真正的 0 同形 —— 本项目反复出现的形态的又一实例。
- 结论对**不能**反推理由对。**答对了也要问一句"我这个数是从哪来的"**，
  尤其当这个数**恰好支持自己**的时候。
...
## [gatekeeper] 承上：那两个夹具已修，但**根因不是上一条写的那样** —— 一个是"夹具比判据旧一版"，一个是"夹具抄了缺陷本身"

- 承接上一条（`test/gate/` 5 个用例红、而报告写"自证全绿"）。修完之后的更正：
- **`p0_gate_judgment_test.dart` 的根因不是"没补 P0.6/P0.1a 的输入"，是整份语料在按 1° 死区写。**
  `goodCorpus()` 里 `applied = |真值| > 1.0 ? 真值 : 0` —— 那是 v1（1° 死区）下"合格实现"的定义。
  v2 把死区放到 10°，同一份语料于是**在 P0.6 眼里处处违规**：12 条锚点里 8 条"死区内被转动"、
  P0.1a 读到 `max|残余−真值| = 8.01`（判据上限 1.5）。**只要补几个字段是修不好的** ——
  必须把"合格"的定义跟着判据改。自检用例 C 同理：它挑的夹具真值 5.6°，在 v1 下在判定集里，
  在 v2 下落在死区内、根本不进 P0.3b 的判定集，于是"该 FAIL"变成了"判空集"。
  用例 A 更微妙：它拿 p1/c03（真值 −4.4/−3.72，`unavailable`）当违规，而 v2 **明文豁免**死区内的
  `unavailable`（口径 2）—— 那半边事故在新口径下不再是事故。**仍违规、也仍然被抓住的是 p2
  （竖直样本被转歪 3.65°）**，现在由 P0.2 + P0.6 两条钉住；豁免那半边改成断言"报告必须写明豁免"，
  否则"豁免生效"与"这一栏根本没判"在报告里长得一模一样。
- **`provenance_test.dart` 的根因不是"没补 `roundValid` 字段"，是夹具把该字段写进了错的那一层。**
  `_writeSummary` 写的是 `provenance.codeFingerprint.roundValid`，而 `_verifyOne` 取的是
  `provenance` 层（`pointer` 去掉末段）——**夹具原样复刻了 `4e33340` 要修的那个缺陷**。
  特征最坏的一点在这里：**夹具跟着缺陷走，于是那条缺陷在自己的自证面上永远是绿的**。
  报告"整轮作废"的产物与"真的整轮作废"在自证面上同形。
  同一次修复里给 `roundValid ?? codeStableDuringRun` 那条 `??` 补了正例 —— 它是"整轮作废"的总开关，
  此前**一个用例都没有**。
- 判据：**修判定实现时，夹具不是"补字段"的宾语，是"跟着改口径"的主语。** 两问缺一不可：
  ①这个夹具描述的是**哪一版**判据下的合格/不合格？②它写的字段**在哪一层**，跟我读的那层是不是同一层？
  第二问尤其阴 —— 字段名写得对、层写错了，夹具会**替缺陷打掩护**。
- 判据（承接上一条）：报告里写"自证全绿"必须**点名是哪几个面跑绿的**。本门禁至少两个面：
  ① `dart run tools/gate/p0_p06_selftest.dart`；② `flutter test test/gate/`。
  **只跑一个面就报"全绿"，与"根本没跑"是同一个形态。**
...
## [imaging] 门禁同款夹具的 `FaceInfo.box` 没跟着画布一起转：小角度兜得住，一放大就"头出画幅"，看着像引擎崩了

- 现象（2026-09-17）：自动摆正门槛 10° → 30°、上限 30° → 90° 后，
  `dev_gate_repro.dart`（门禁同款夹具）在 **+45°** 上打出「青色像素=0」，而 −45° 正常。
  一眼像"引擎在大角度下把裁剪框摆飞了"。
- 根因在**夹具**：`buildGateSynthetic` 用 `img.copyRotate` 转画布（画布还会被撑大，
  1000×1400 → 1697×1697），但 `FaceInfo.box` 写的是 `Rect.fromLTWH(kRectX0, headTopY,
  kRectX1−kRectX0, …)` —— **未旋转的设计坐标 350–650**。契约规定 box 与抠图同坐标系，
  引擎据 `box.center.dx` 定水平锚点，于是锚点偏量随角度增长。实测青条真实质心 x：

  | 输入角 | 画布 | box.x | 青条真实质心 x | 青条在裁剪框里的归一化 x |
  |---|---|---|---|---|
  | +15° | 1328×1611 | 350–650 | 766.8 | 0.886（勉强在框内）|
  | −15° | 1328×1611 | 350–650 | 560.7 | 0.352 |
  | +45° | 1697×1697 | 350–650 | 1129.8 | **1.916（出画幅）** |
  | −45° | 1697×1697 | 350–650 | 566.2 | **−1.825（出画幅）** |

  偏心 +15° 就有 267px、−15° 有 61px，只是那个画幅还兜得住；门槛一放开到 30° 以上，
  夹具角必然 >30°，就兜不住了。引擎侧自建夹具（box 严格等于旋转后的头中心）在 ±45°/±60°
  上的残差是 0.000°/0.076° —— **同一份引擎代码，两个夹具，结论相反**。
- 判据：**"门禁同款"这四个字只保证口径同源，不保证夹具正确。** 改判据的适用域（门槛角度）
  时，要连夹具**有效域**一起扫：这个夹具在哪些角度上还是它所声称的那个模型？
  本条的失效模式尤其阴 —— **夹具跟着产品常量走，于是它在新角度上测出来的偏差会被记在实现方头上**。
- 处理：`dev_gate_repro.dart` 的注释里留了上面这张表和"这不是引擎判决"的运行时横幅；
