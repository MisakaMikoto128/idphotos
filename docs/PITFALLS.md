...
**量化步长（本轮最该记住的数）**：在 `cn_big_1inch`（`eyedist ≈ 90 px`）上，
**1 像素的眼距-法向位移 = 0.6366°** ⇒ 该量具的**步长就是 0.6366°**。
P0.2 报的 `max|残余| = 0.637` **恰好是一个步长**，10 条读数（0.454–0.637）**全 ≤ 一个步长**，
即这支量具**无法把它们与 0 区分**。判决没翻（都 ≤1.5°），但"上界 = 0.637"必须读作"= 1 像素"。
