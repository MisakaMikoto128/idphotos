...
| P0.6 | **死区内不得被转动** | 真值 \|tilt\| ≤ 10° 的样本上，**\|成片残余 − 真值\| ≤ 1.5°**——引擎必须原样保留这张照片的倾角，转正与转歪**都是 FAIL**。适用范围：锚点、`straight`、`uprightSynthetic`、以及死区内的夹具。**本条替代本次事故的原始检测面**：`2.jpg`（真值 −0.2°）被转歪 3.7° 在本口径下报 **3.5°**，照样 FAIL；P0.2 是本条真值 ≈ 0 的特例。 |
