...
| # | 检查项 | 阈值 |
|---|---|---|
| P0.1 | 真值锚点残差 | 锚点集 ≥ 8 张真实照片，真值由 ≥ 2 种互相独立的测量 + 目视复核标定（证据落 `out/P0_*`）。最终成片残余 \|residual\| ≤ 1.5°，**且方向正确**（不得靠"不转"蒙对方向） |
| P0.2 | **不引入歪斜** | 已知竖直样本（用户 `2.jpg` + 锚点回正合成样本）上，最终成片残余 ≤ 1.5°。当前实现为 +3.7°——本项就是这条事故的判据 |
| P0.3 | 旋转等变 | 对锚点施加已知 Δ ∈ {±3, ±5, ±10}°，估计输出随 Δ 线性变化：斜率 ∈ [0.85, 1.15]，无方向性衰减 |
| P0.4 | 覆盖率与诚实性 | 报告 `RollSource.pupil` 在真实语料（Pictures 全量 + 黄金集 8 + 锚点）上的成功率；`unavailable` 样本的 `rollDeg` 必须 = 0.0，**禁止**回退到已知有 3.7–6.7° 系统误差的 YuNet 眼睑路径 |
