...
| P0.1b | **条件覆盖率** | 真值 \|tilt\| > 2° 的锚点上，`unavailable` 必须 = **0**——"该摆正的照片不许说测不出"。真值 \|tilt\| ≤ 2° 的样本返回 `unavailable` 可接受（本就在死区附近，不转无害），单独报数、不入判据。 |
...
| P0.3b | 旋转等变（覆盖率） | 真值 \|tilt\| > 2° 的旋转夹具上，`unavailable` 必须 = **0**。 |
...
2. **但 `unavailable` 也不是一律有罪**：真正测不出的样本（暗光、闭眼、多人小脸）返回 0 是诚实行为，好过给一个错的角。所以判据落在**条件覆盖率**（P0.1b / P0.3b）：**只要这张图确实需要摆正（\|tilt\| > 2°），就必须给出 `pupil` 估计**；不需要摆正的图返回 `unavailable` 无害。
