...
6. **锚点去重**：qa-batch 报告 12 条倾斜锚点里 `c03`/`c04` 是同一张照片的不同分辨率副本（逐像素 mean\|diff\|=2.74），`c10`/`c11`/`c12` 是同一张五人合影的不同裁切——**实际只覆盖 11 张不同照片**。仍满足"≥8 张"的要求，但报告与判据里的样本数一律按 **11 张不同照片**表述，不得用 12/13 的计数虚增覆盖。
   - **2026-09-17 更正（本轮 r2 开跑前的**事实**订正，非阈值变更）**：其中 `c10`/`c11`/`c12` 那半句**不成立**。qa-batch 用 ECC 欧氏对齐 + **只统计结构像素（Sobel 梯度 > 25）**复核（平坦蓝底会把相似度整体抬高，必须剔除），并以已知同图对 `c03`/`c04` 标定两端：
     | 对 | 结构像素 ≤5 灰阶 | 中位差 |
     |---|---|---|
     | `c03`/`c04`（已知同图） | **95.2%** | 1 |
     | `c10`/`c11` | 11.3% | 33 |
     | `c10`/`c12` | 10.6% | 36 |
     差一个数量级；三条均为 **4032×3024 全画幅**、取景与站位不同（`out/P0_anchors/c10_c11_c12_sheet.png`）。**它们是三张不同的照片，不是同一张的裁切。** `c03`/`c04` 那半句经复核**成立**（scale=0.4165 恰为分辨率比、相对旋转 −0.024°、结构像素 ≤5 = 95.2%）。另排查并排除 `c07`↔`p2`（全图相似度一度很高，限制到结构像素后仅 24.2%、中位 13 → 同一人同一蓝底的不同次拍摄，非同一张）。
     **故不同照片数为 12，不是 11**（`anchors` 13 条 = 13 个 distinct id；`p2` 同现于 `anchors` 与 `straight`，同一文件同一真值，不新增照片；减 `c03`/`c04` 这一对真重复 = **12**）。
     ≥8 张的要求在新旧计数下**都满足**，**本条不改变任何阈值、不改变任何样本的判据归属**，只订正事实与计数。r2 开跑前生效。
