...
def gate_inputs(truth, resid):
    ...
    return {
        ...
        "deadZone": {
            ...
            "reverseCheck": {
                ...
                "pupilClampedOutsideDeadZone": sorted(
                    ...
                    and r.get("straightenDeg") in (0.0, 0, None)
                    and r.get("rollSource") == "pupil"),
                "conclusion": "**空集** —— 没有一条 `pupil` 在死区之外被静默钳零。"
                              "即「测得准却被钳零」的唯一成因是死区，"
                              "**不存在第二条隐藏通道**。正反两条合起来才说明"
                              "钳制行为自洽。",
            },
            "whyHarmless": "死区 1.0 ≤ P0.1a 容差 1.5：被钳到 0 的样本其残余"
                           "天然过线，因此死区本身不制造 P0.1a 违规。",
        ...
        "scoringDenominators": {
            ...
            "distinctAnchorNote": "第 6 条：12 条 anchor 去重后实际 **11 张不同照片**"
                                  "（c03=c04 同图两分辨率；c10/c11/c12 同合影三裁切）。"
                                  "报告与判据一律按 11 张表述。",
            "P0.1a": {
