...
const Map<int, List<String>> kRoundErrata = <int, List<String>>{
  ...
  4: <String>[
    ...
    '8. **`out/GATE_P0_r3.md:36` 已被取代（本轮必须点这一句）。** 那一行写着'
        '「ml-porting 有 3 次修复机会，对应门禁运行 **r2 / r3 / r4**；r4 仍 FAIL → 写 '
        '`out/BLOCKED_G2B-P0.md`」，读起来是「**名额 = 轮次**」——**这是错的**。'
        '它与文末补记 6 相隔 700 多行，**单读 :36 会读出错误的记账口径**，'
        '正是本项目那个特征失效形态（名字声称的口径宽于它实际钉住的范围）。'
        '处理：归档件正文**一字未删**，在该行**紧接加一条取代标记**指向补记 6 与本轮账本；'
        '本轮报告同时点明它已被取代。⚠ 该文件是**生成物** —— '
        '谁若重跑 `--round 3`，那条行内标记会被冲掉。',
  ],
  // r3 的更正**必须走这里，不能只写在 md 里**：`out/GATE_P0_r3.md` 是生成物，
  // 只加行内标记的话，**重跑一次 `--round 3` 那段更正会静默消失**，文件回到
  // "读起来正常却会读错"——正是本项目那个特征失效形态。
  // （自证方法：在系统临时目录重跑 `--round 3`，确认本段出现在生成物里。
  //  证据只报"出现/没出现"，那次运行的任何**读数都不得引用**。）
  3: <String>[
    '### 事后更正（2026-09-17 主会话裁定；不推翻下方任何原始结论，只标出记账口径已被取代）',
    '1. **本文件「轮次记账」一节里的这句是错的**：'
        '「ml-porting 有 3 次修复机会，对应门禁运行 **r2 / r3 / r4**；r4 仍 FAIL → 写 '
        '`out/BLOCKED_G2B-P0.md`」。它读起来是「**名额 = 轮次**」。'
        '**正确口径**（主会话裁定；全文见 r4 报告与 `tools/gate/gate_P0.dart` 的 `kRoundLedgerMd`）：'
        '① **预算是按实现方分账的**，不是全局池；② **只在真的 FAIL 时消耗**'
        '（作废轮、被判边界噪声的轮都不消耗）；③ **"最后一轮" ≠ "预算用尽"**；'
        '④ **失败项按根因逐项归属** —— 死区与裁剪回退是 `lib/core/imaging/` 的代码，'
        '记 imaging 的账，**不许一律记在 ml-porting 头上**。',
    '2. **为什么单读那一行会读错**：它与文末的补记 6 相隔 700 多行。'
        '补记 6 已经把口径覆盖了，但**只读「轮次记账」那一节的人看不到它** —— '
        '**名字声称的口径宽于它实际钉住的范围，而它读起来完全正常。**'
        '故此处再写一遍；归档件正文**一字未删**。',
    '3. **本轮判决本身不受影响**：更正的是**记账口径**，不是任何一条判据的结论。'
        'r1 / r2 / r3 的原始数字与结论原样有效，只作**沿革**用；'
        '按主会话裁定，判据已改为冻结于 `d4fb782` 的 v2（10° 死区分档 + 新增 P0.6），'
        '**r1–r3 的任何 PASS/FAIL 不得当作 r4 及以后的读数引用**。',
    '4. **本文件是生成物**：`out/GATE_P0_r3.md` 由 `tools/gate/gate_P0.dart` 写出。'
        '归档件里另有一条**紧贴错误行的行内标记**（重跑会冲掉）；本段则**每次重跑都在**。'
        '两者互补：行内标记管"只读那一节的人"，本段管"重跑之后"。',
  ],
};
...
String kRoundLedgerMd(int round) {
  ...
  b.writeln();
  // 防漏洞条款：没有它，"一律作废"能把 3 轮预算变成无限。
  b.writeln('**防漏洞条款（主会话 2026-09-17 裁定，与上表同为记账口径的一部分）**：'
      '作废轮**只在两种情形成立** —— ① **门禁自身缺陷**（判据实现错、脚本崩、'
      '量具未自证）；② **产物不可信**（写入窗口冲突、指纹不符、输入早于被测代码）。'
      '**且作废必须由主会话认定并写明原因**，门禁不得自行宣布。'
      '**凡因实现方代码不达标而 FAIL 的轮次，一律消耗**，'
      '不论它失败得多"擦边"。理由：**「一律作废」能把 3 轮预算变成无限** —— '
      '这是这条记账原则唯一的漏洞面，明写在案比事后追认干净。'
      '（r3 之所以不消耗，走的是②：两份被判产物早于修复落地，'
      '**输入不可信**，且由主会话认定为无效轮。）');
  return b.toString();
}
